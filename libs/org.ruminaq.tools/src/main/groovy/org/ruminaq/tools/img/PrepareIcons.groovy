...
class PrepareIcons {
	...
	def execute(params) {
		...
		json.xcfs.each {
			...
			process.waitFor()
			if(cmds.contains('diagram')) { process = """convert -geometry x30 ${png.absolutePath} ${icons.absolutePath}/diagram.${fileName}.png""".execute() ; process.waitFor() }
			...
			if(cmds.contains('palette')) { process = """convert -geometry x22 ${png.absolutePath} ${icons.absolutePath}/palette.${fileName}.png""".execute() ; process.waitFor() }
			if(cmds.contains('icon'))    { process = """convert -geometry x30 ${png.absolutePath} ${html_img.absolutePath}/icon.png""".execute() ; process.waitFor() }
