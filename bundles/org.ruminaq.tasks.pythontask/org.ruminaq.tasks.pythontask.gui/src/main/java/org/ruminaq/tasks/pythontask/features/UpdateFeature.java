...
import org.ruminaq.model.sic.UserDefinedTask;
...
public class UpdateFeature extends UpdateUserDefinedTaskFeature {
    ...
    public boolean load(String className) {
        ...
        this.desc = this.sourceModule == null ? AddFeature.NOT_CHOSEN : className.substring(i != -1 ? i + 1 : 0);
    ...
    protected void loadInputPorts() {
        ...
        while(it.hasNext()){
            ...
            if(entry.node instanceof FunctionDef) {
                ...
                if(functionDef.name instanceof NameTok && ((NameTok) functionDef.name).id.equals("__init_ports__")) {
                    for(stmtType t : functionDef.body) {
                        if(t instanceof Assign) {
                            Assign a = (Assign) t;
                            if(a.value instanceof Call) {
                                Call c = (Call) a.value;
                                if(c.func instanceof Name && ((Name) c.func).id.equals("InputPort")) {
                                    String   name         = null;
                                    ...
                                    if(c.keywords != null)
                                        for(keywordType k : c.keywords) {
                                            if(k.arg instanceof NameTok && ((NameTok) k.arg).id.equals("name")) name = ((Str) k.value).s;
                                            if(k.arg instanceof NameTok && ((NameTok) k.arg).id.equals("data_type")) {
                                                if(k.value instanceof Name) {
                                                    ...
                                                    if(l.elts != null) {
                                                        ...
                                                        for(int i = 0; i < dataType.length; i++) dataType[i] = ((Name) l.elts[i]).id;
                                                    }
                                                }
                                            }
                                            if(k.arg instanceof NameTok && ((NameTok) k.arg).id.equals("asynchronous")) asynchronous = Boolean.parseBoolean(((Name) k.value).id);
                                            if(k.arg instanceof NameTok && ((NameTok) k.arg).id.equals("group"))        group        = Integer.parseInt    (((Num) k.value) .num);
                                            if(k.arg instanceof NameTok && ((NameTok) k.arg).id.equals("hold"))         hold         = Boolean.parseBoolean(((Name) k.value).id);
                                            if(k.arg instanceof NameTok && ((NameTok) k.arg).id.equals("queue"))        queue        = Integer.parseInt    (((Num) k.value) .num);
                                        }
                ...
                if(functionDef.name instanceof NameTok && ((NameTok) functionDef.name).id.equals("__init_ports__")) {
                    for(stmtType t : functionDef.body) {
                        if(t instanceof Assign) {
                            ...
                            if(a.value instanceof Call) {
                                Call c = (Call) a.value;
                                if(c.func instanceof Name && ((Name) c.func).id.equals("OutputPort")) {
                                    String   name     = null;
                                    String[] dataType = new String[] { "Control" };
    ...
    protected Map<String, String> getParameters(UserDefinedTask udt) {
        ...
        while(it.hasNext()){
            ...
            if(entry.node instanceof ClassDef){
                ...
                if(ds == null) continue;
                for(decoratorsType d : ds)
                    if(d.func instanceof Name && ((Name) d.func).id.equals("SicParameter")) {
                        String name = "";
                        String defaultValue = "";
                        for(keywordType k : d.keywords) {
