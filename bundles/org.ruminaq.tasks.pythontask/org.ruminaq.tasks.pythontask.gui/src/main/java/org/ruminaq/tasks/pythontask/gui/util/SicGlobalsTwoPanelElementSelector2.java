...
public class SicGlobalsTwoPanelElementSelector2
    extends GlobalsTwoPanelElementSelector2 {

  private PythonNature nature;
  private IModulesManager modulesManager;

  public SicGlobalsTwoPanelElementSelector2(Shell shell, boolean multi,
      String selectedText, IProject p) {
  ...
  protected ItemsFilter createFilter() {
    return new InfoFilter() {
      ...
      public boolean matchItem(Object item) {
        ...
        if (info.info.getType() != IInfo.CLASS_WITH_IMPORT_TYPE)
          return false;
        if (MatchHelper.matchItem(patternMatcher, info.info)) {
          IModule module = modulesManager
              .getModule(info.info.getDeclaringModuleName(), nature, true);
