...
public class CreatePythonTaskWizard extends PythonModuleWizard  {
    ...
    public void setListener(CreatePythonTaskListener listener) { this.listener  = listener; }
    ...
    protected void afterEditorCreated(final IEditorPart openEditor) {
        if(!(openEditor instanceof PyEdit)) return;
        PyEdit pyEdit = (PyEdit) openEditor;
        if(pyEdit.isDisposed()) return;
        ...
        Display.getDefault().syncExec(new Runnable() {
            ...
            public void run() {
                ...
                try { pyEdit.getIFile().setContents(IOUtils.toInputStream(code, "UTF-8"), IResource.FORCE, new NullProgressMonitor());
    ...
    public boolean performFinish() {
        ...
        String pack = filePage.getValidatedPackage() != null ?
                      filePage.getValidatedPackage().toString().replace(filePage.getValidatedSourceFolder().toString(), "") :
                      filePage.getPackageText() != null ? filePage.getPackageText() : "";
        if(pack.startsWith("/")) pack = pack.substring(1);
        pack = pack.replace("/", ".");
        if(!"".equals(pack)) pack += ".";
        if(listener != null) listener.created(pack + filePage.getValidatedName());
        return ret;
    }
}
