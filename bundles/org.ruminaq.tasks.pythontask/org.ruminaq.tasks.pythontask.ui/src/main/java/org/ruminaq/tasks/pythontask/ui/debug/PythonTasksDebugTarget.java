...
public class PythonTasksDebugTarget extends PythonTasksDebugElement implements IDebugTarget, ISicTarget {
    ...
    private final Logger logger = ModelerLoggerFactory.getLogger(PythonTasksDebugTarget.class);

    protected EventDispatchJob  dispatcher;
    protected ILaunch           launch;
    protected IProject          project;
    protected PythonTasksProcess  process;
    protected List<PythonTask> tasks = Collections.synchronizedList(new LinkedList<PythonTask>());

    @Override public PythonTasksDebugTarget getDebugTarget() { return this; }

    @Override public void setState(IState state) { super.setState(state); }

    public PythonTasksDebugTarget(ILaunch launch, IProject project, EventDispatchJob dispatcher) {
        super(null);
        this.launch     = launch;
