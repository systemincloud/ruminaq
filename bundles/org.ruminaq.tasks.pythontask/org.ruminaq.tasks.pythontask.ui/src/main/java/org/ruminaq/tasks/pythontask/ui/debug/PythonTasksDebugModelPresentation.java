...
public class PythonTasksDebugModelPresentation
  ...
  public static Image getImage(String file) {
    Bundle bundle = FrameworkUtil
        .getBundle(PythonTasksDebugModelPresentation.class);
  ...
  public Image getImage(Object element) {
    if (element instanceof PythonTasksDebugTarget) {
      PythonTasksDebugTarget target = (PythonTasksDebugTarget) element;
      if (target.isTerminated())
        return TARGET_TERMINATED;
      if (target.hasSuspended())
        return TARGET_PARTLY_SUSPENDED;
      return TARGET_RUNNING;
    } else if (element instanceof PythonTask) {
      PythonTask task = (PythonTask) element;
      if (task.isSuspended())
