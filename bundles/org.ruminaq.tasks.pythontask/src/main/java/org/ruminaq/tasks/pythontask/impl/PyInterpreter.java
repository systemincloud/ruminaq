...
public abstract class PyInterpreter implements IPyInterpreter {
    ...
    public synchronized static PyInterpreter getInstance(int type, EmbeddedTaskI parent, String processJar) {
        PyInterpreter ret;
        if(type == IPythonNature.INTERPRETER_TYPE_PYTHON) ret = CPythonInterpreter.getInstance();
        else                                              ret = JythonInterpreter .getInstance();
        if(!ret.initialized) {
	...
	@Override
	public synchronized PythonTaskProxy createTask(String implFile, String id, PythonTaskI pythonTaskI) {
