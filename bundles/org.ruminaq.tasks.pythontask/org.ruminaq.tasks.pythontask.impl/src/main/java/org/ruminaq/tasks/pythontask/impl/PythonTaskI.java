...
public class PythonTaskI extends GeneratorI implements PythonTaskListener {
    ...
    private final Logger logger = RunnerLoggerFactory.getLogger(PythonTaskI.class);
    ...
    public PythonTaskI(EmbeddedTaskI parent, Task task) {
        ...
        String processJar = mvnRepo + File.separator + PROCESS_LIB_GROUPID.replace(".", File.separator)
                                    + File.separator + PROCESS_LIB_ARTIFACT
                                    + File.separator + PROCESS_LIB_VERSION
                                    + File.separator + PROCESS_LIB_ARTIFACT + "-" + PROCESS_LIB_VERSION + ".jar";
        ...
        // Parameters
        //
        for(Entry<String, String> p : ((org.ruminaq.tasks.pythontask.model.pythontask.PythonTask) task).getParameters().entrySet())
            parameters.put(p.getKey(), parent.replaceVariables(p.getValue()));

        this.pyProxy = interpreter.createTask(impl, Util.getUniqueId((PythonTask) task, parent.getBasePath()), this);
    ...
    @Override public void runnerStart() { pyProxy.runnerStart(); }
    @Override public void runnerStop()  { pyProxy.runnerStop(); }
    ...
    @Override public DataI getData(String ipName) {
