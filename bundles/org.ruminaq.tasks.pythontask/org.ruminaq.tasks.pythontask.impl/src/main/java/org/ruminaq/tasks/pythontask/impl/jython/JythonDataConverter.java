...
public enum JythonDataConverter {
  ...
  public PyObject toJythonData(PythonInterpreter pi, DataI dataI) {
    ...
    PyObject data = PythonRunnerServiceManager.INSTANCE.toJythonData(pi, dataI,
        pyDims);
    if (data != null)
      return data;
  ...
  public DataI fromJythonData(PythonInterpreter pi, PyObject data) {
    ...
    DataI dataI = PythonRunnerServiceManager.INSTANCE.fromJythonData(pi, data,
        pyValues, dims);
    ...
    if (dataI != null)
      ...
      for (PyObject v : pyValues)
        bd[i++] = new BigDecimal(
            ((PyString) v.__getattr__("__str__").__call__()).asString());
      ...
      for (PyObject v : pyValues) {
        if (v instanceof PyLong)
          values[i++] = ((PyLong) v).asInt();
        else if (v instanceof PyInteger)
          values[i++] = ((PyInteger) v).asInt();
      }
      ...
      for (PyObject v : pyValues) {
        if (v instanceof PyLong)
          values[i++] = ((PyLong) v).asLong();
        else if (v instanceof PyInteger)
          values[i++] = ((PyInteger) v).asLong();
      }
