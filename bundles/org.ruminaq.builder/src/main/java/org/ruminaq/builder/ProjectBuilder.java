...
public enum ProjectBuilder {
    ...
    private final Logger logger = ModelerLoggerFactory.getLogger(ProjectBuilder.class);
    ...
    public void build(IProject project) throws CoreException {
        ...
        if(result != 0) throw new CoreException(Status.CANCEL_STATUS);
    }
    ...
    public File getBuiltArtifact(IProject project) {
        ...
        try {
            ...
            String path = project.getLocation().toOSString() + "/target/" + model.getArtifactId() + "-" + model.getVersion() + ".jar";
            file = new File(path);
