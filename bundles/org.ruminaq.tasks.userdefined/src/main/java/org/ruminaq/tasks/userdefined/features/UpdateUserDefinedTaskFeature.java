...
public abstract class UpdateUserDefinedTaskFeature extends UpdateTaskFeature {
    ...
    private final Logger logger = ModelerLoggerFactory.getLogger(UpdateUserDefinedTaskFeature.class);
	...
	protected final class FileInternalInputPort {
		...
		public FileInternalInputPort(String name, List<DataType> dataType, boolean asynchronous, int group, boolean hold, String queue) {
			this.name         = name;
		...
		List<DataType>              getDataType()        { return dataType; }
		boolean                     isAsynchronus()      { return asynchronous; }
		int                         getGroup()           { return group; }
		boolean                     isHold()             { return hold; }
		String                      getQueue()           { return queue; }
		Class<? extends DataType>[] getDataTypeClasses() {
			Class<?>[] ret = new Class<?>[dataType.size()];
			int i = 0;
			for(DataType dt : dataType) ret[i++] = dt.getClass();
	...
	private boolean compareInputPorts(List<FileInternalInputPort> inputs, List<InternalInputPort> inputPorts) {
		if(inputs.size() != inputPorts.size()) return false;
		loop:for(FileInternalInputPort fip : inputs) {
			for(InternalInputPort iip : inputPorts)
				if(    fip.getName().equals(iip.getId())
					...
					&& fip.getQueue().equals(iip.getDefaultQueueSize()) ) continue loop;
			return false;
		}
		return true;
	}

	private boolean compareOutputPorts(List<FileInternalOutputPort> outputs, List<InternalOutputPort> outputPorts) {
		if(outputs.size() != outputPorts.size()) return false;
		loop:for(FileInternalOutputPort fip : outputs) {
			for(InternalOutputPort iop : outputPorts)
				if(fip.getName().equals(iop.getId()) && ModelUtil.areEquals(fip.getDataType(), iop.getDataType())) continue loop;
			return false;
		}
		return true;
	}
	...
	public IReason updateNeeded(IUpdateContext context) {
		...
		if("".equals(resource)) { loadIconDesc(); return Reason.createFalseReason(); }
		if(!load(resource))     { loadIconDesc(); return Reason.createFalseReason(); }
		loadIconDesc();
		loadInputPorts();
		loadOutputPorts();
		loadAtomic();
		...
		this.onlyLocalUpdateNeeded  = onlyLocalDefault ? onlyLocal != this.onlyLocal : false;
		this.paramsUpdateNeeded     = !compareParams(((UserDefinedTask) task));

		boolean updateNeeded = this.superUpdateNeeded
				            || this.descUpdateNeeded
				            || this.inputsUpdateNeeded
							|| this.outputsUpdateNeeded
							|| this.atomicUpdateNeeded
							|| this.onlyLocalUpdateNeeded
	...
	public boolean update(IUpdateContext context) {
		...
		if(onlyLocalUpdateNeeded) updated = updated | onlyLocalUpdate(parent, be);
	...
	private boolean descUpdate(ContainerShape parent, Task be) {
		if(iconDesc != null)
			for(GraphicsAlgorithm ga : parent.getGraphicsAlgorithm().getGraphicsAlgorithmChildren())
				if(Graphiti.getPeService().getProperty(ga, AddTaskFeature.ICON_DESC_PROPERTY) != null) {
	...
	private boolean inputsUpdate(ContainerShape parent, Task task) {
			...
			for(FileInternalInputPort fip : inputs)
				if(fip.getName().equals(iip.getId())) {
					...
					if(!ModelUtil.areEquals(fip.getDataType(), iip.getDataType())) {
						while(iip.getDataType().size() > 0) iip.getDataType().remove(0);
						iip.getDataType().addAll(fip.getDataType());
					}
					if(fip.isAsynchronus() != iip.isAsynchronous()) {
						iip.setAsynchronous(fip.isAsynchronus());
						if(iip.isAsynchronous()) AddTaskFeature.getPictogramElementOfInternalPort(getDiagram(), iip).getGraphicsAlgorithm().getGraphicsAlgorithmChildren().get(0).setLineStyle(LineStyle.DOT);
						else                     AddTaskFeature.getPictogramElementOfInternalPort(getDiagram(), iip).getGraphicsAlgorithm().getGraphicsAlgorithmChildren().get(0).setLineStyle(LineStyle.SOLID);
					}
					if(fip.getGroup() != iip.getGroup()) {
						iip.setGroup(fip.getGroup());
					}
					if(fip.isHold() != iip.isDefaultHoldLast()) {
						if(iip.isDefaultHoldLast() == iip.isHoldLast()) iip.setHoldLast       (fip.isHold());
						                                                iip.setDefaultHoldLast(fip.isHold());
					}
					if(fip.getQueue() != iip.getDefaultQueueSize()) {
						if(iip.getDefaultQueueSize().equals(iip.getQueueSize())) iip.setQueueSize(fip.getQueue());
		...
		for(InternalInputPort iip : inputsToRemove) removePortShape(task, parent, iip);

		loop:for(FileInternalInputPort fip : inputs) {
			for(InternalInputPort iip : inputPorts) if(fip.getName().equals(iip.getId())) continue loop;
			addInputPort(task, parent, fip.getName(), true, fip.getDataTypeClasses(), fip.isAsynchronus(), fip.getGroup(), fip.isHold(), fip.getQueue(), Position.LEFT);
		}
		return true;
	}
	...
	private boolean outputsUpdate(ContainerShape parent, Task task) {
			...
			for(FileInternalOutputPort fip : outputs)
				if(fip.getName().equals(iop.getId())) {
					if(!ModelUtil.areEquals(fip.getDataType(), iop.getDataType())) {
						while(iop.getDataType().size() > 0) iop.getDataType().remove(0);
		...
		for(InternalOutputPort iop : outputsToRemove) removePortShape(task, parent, iop);

		loop:for(FileInternalOutputPort fip : outputs) {
			for(InternalOutputPort iop : outputPorts) if(fip.getName().equals(iop.getId())) continue loop;
			addOutputPort(task, parent, fip.getName(), true, fip.getDataTypeClasses(), Position.RIGHT);
		}
		return true;
	}
	...
	private boolean atomicUpdate(ContainerShape parent, Task be) {
		be.setAtomic(atomic);
		if(atomic) parent.getGraphicsAlgorithm().getGraphicsAlgorithmChildren().get(0).setLineStyle(LineStyle.SOLID);
		else       parent.getGraphicsAlgorithm().getGraphicsAlgorithmChildren().get(0).setLineStyle(LineStyle.DOT);
		return true;
	}
	...
	private boolean onlyLocalUpdate(ContainerShape parent, Task task) {
		if(task.isOnlyLocalDefault()) task.setOnlyLocal(onlyLocal);
	...
	private boolean paramsUpdate(ContainerShape parent, Task be) {
		...
		for(String s : is)
			if(!shouldBe.keySet().contains(s)) toRemove.add(s);
		for(String s : toRemove) udt.getParameters()       .remove(s);
		for(String s : toRemove) udt.getDefaultParameters().remove(s);

		List<String> toAdd = new LinkedList<>();
		for(String s : shouldBe.keySet())
			if(!is.contains(s)) toAdd.add(s);
		for(String s : toAdd) udt.getParameters()       .put(s, shouldBe.get(s));
		for(String s : toAdd) udt.getDefaultParameters().put(s, shouldBe.get(s));
