...
public abstract class CreateUserDefinedTaskPage extends WizardPage implements ICreateUserDefinedTaskPage {
    ...
    static class RowTransfer extends ByteArrayTransfer {
        ...
        public  static RowTransfer getInstance () {	return instance; }
        @Override protected String[] getTypeNames() { return new String[] { ROWTYPENAME }; }
        @Override protected int[]    getTypeIds()   { return new int[]    { ROWTYPEID }; }
        @Override public void        javaToNative(Object object, TransferData transferData) { }
        @Override public Object      nativeToJava (TransferData transferData) { return null; }
    }
    ...
    private void initLayout(Composite parent) {
        ...
        grpGeneral                 .setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false, false, 2, 1));
        ...
        grpRunner                  .setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false, false, 2, 1));
        ...
        lblInputPorts              .setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false, false, 2, 1));
        ...
        tblInputsDropTrg      = new DropTarget(tblInputs, DND.DROP_MOVE | DND.DROP_DEFAULT);
        ...
        lblInputsAddName           .setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
        ...
        lblOutputPorts             .setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false, false, 2, 1));

        tblOutputs            = new Table(root, SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI);
        tblOutputs                 .setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true, 1, 2));
        ...
        tblOutputsDropTrg     = new DropTarget(tblOutputs, DND.DROP_MOVE | DND.DROP_DEFAULT);
        ...
        lblOutputsAddName          .setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
        ...
        tblParameters         = new Table(root, SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI);
        tblParameters              .setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true, 1, 2));
        ...
        lblParametersAddName       .setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
        txtParametersAddName  = new Text  (grpParametersAdd, SWT.BORDER);
        lblParametersAddValue = new Label (grpParametersAdd, SWT.NONE);
        lblParametersAddValue      .setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
    ...
    private void initComponents() {
        ...
        for(TableColumn tc : tblInputs.getColumns()) tc.pack();
        ...
        for(TableColumn tc : tblOutputs.getColumns()) tc.pack();
        ...
        for(TableColumn tc : tblParameters.getColumns()) tc.pack();
    ...
    private void initActions() {
        btnAtomic.addSelectionListener(new SelectionAdapter() {
            @Override public void widgetSelected(SelectionEvent event) {
                if(!btnAtomic.getSelection()) { btnConstant.setSelection(false); }
            }});
        ...
        btnGenerator.addSelectionListener(new SelectionAdapter() {
            @Override public void widgetSelected(SelectionEvent event) {
                if(btnGenerator.getSelection()) { btnConstant.setSelection(false); }
            }});
        btnExternalSource.addSelectionListener(new SelectionAdapter() {
            @Override public void widgetSelected(SelectionEvent event) {
                if(btnExternalSource.getSelection()) { btnConstant .setSelection(false); }
            }});
        tblInputs.addSelectionListener(new SelectionAdapter() {
            @Override public void widgetSelected(SelectionEvent event) { btnInputsRemove.setEnabled(true); }
        });
        tblInputsDragSrc.addDragListener(new DragSourceAdapter() {
            @Override public void dragStart(DragSourceEvent event) {
                ...
                for(int i = 0; i < is.length; i++)
                    if(i > 0 && (is[i] - is[i - 1]) != 1) {	event.doit = false;	return;	}
                event.doit = true;
            }});
        tblInputsDropTrg.addDropListener(new DropTargetAdapter() {
            @Override public void dragOver(DropTargetEvent event) { event.feedback = DND.FEEDBACK_SELECT | DND.FEEDBACK_SCROLL; }
            @Override public void drop    (DropTargetEvent event) {
                DropTarget target = (DropTarget) event.widget;
                Table table = (Table) target.getControl();
                if(table != tblInputs) return;
                ...
                int idx = -1;
                int i = 0;
                for(TableItem it : tblInputs.getItems()) { if(it == ti) { idx = i; break; } i++; }
                if(i == -1 || i >= tblInputs.getItems().length) return;

                i = idx + 1;
                for(TableItem it : items) { new TableItem(tblInputs, SWT.NONE, i).setText(new String[] { it.getText(0),
                                                                                                         it.getText(1),
                                                                                                         it.getText(2),
                                                                                                         it.getText(3),
                                                                                                         it.getText(4),
                                                                                                         it.getText(5)}); i++; }
                for(TableItem it : items) it.dispose();
                tblInputs.redraw();
            }});
        txtInputsAddName.addModifyListener(new ModifyListener() {
            @Override public void modifyText(ModifyEvent event) {
                ...
                for(TableItem it : tblInputs.getItems())
                    if(it.getText(0).equals(txtInputsAddName.getText())) exist = true;
                if("".equals(txtInputsAddName.getText()) || exist) btnInputsAdd.setEnabled(false);
                else                                               btnInputsAdd.setEnabled(true);
            }});
        ...
        btnInputsAdd.addSelectionListener(new SelectionAdapter() {
            @Override public void widgetSelected(SelectionEvent event) {
                ...
                item.setText(new String[] { txtInputsAddName.getText(),
                                            cmbInputsAddData.getText(),
                                            Boolean.toString(async),
                                            async ? "-1" : grp,
                                            async ? Boolean.toString(false) : Boolean.toString(btnInputsAddHold.getSelection()),
                                            inf ? Constants.INF : Integer.toString(spnInputsAddQueue.getSelection())
                    });
                for(TableColumn tc : tblInputs.getColumns()) tc.pack();
                tblInputs.layout();
        ...
        btnInputsRemove.addSelectionListener(new SelectionAdapter() {
            @Override public void widgetSelected(SelectionEvent event) {
                ...
                if(selectionIds.length != 0) btnInputsRemove.setEnabled(false);
        ...
        tblOutputs.addSelectionListener(new SelectionAdapter() {
            @Override public void widgetSelected(SelectionEvent event) { btnOutputsRemove.setEnabled(true); }
        });
        ...
        tblOutputsDropTrg.addDropListener(new DropTargetAdapter() {
            ...
            @Override public void drop    (DropTargetEvent event) {
                ...
                int idx = -1;
                int i = 0;
                for(TableItem it : tblOutputs.getItems()) { if(it == ti) { idx = i; break; } i++; }
                if(i == -1 || i >= tblInputs.getItems().length) return;

                i = idx + 1;
                for(TableItem it : items) { new TableItem(tblOutputs, SWT.NONE, i).setText(new String[] { it.getText(0),
                                                                                                          it.getText(1),
                                                                                                          it.getText(2)}); i++; }
                for(TableItem it : items) it.dispose();
                tblOutputs.redraw();
        ...
        txtOutputsAddName.addModifyListener(new ModifyListener() {
            @Override public void modifyText(ModifyEvent event) {
                ...
                for(TableItem it : tblOutputs.getItems())
                    if(it.getText(0).equals(txtOutputsAddName.getText())) exist = true;
                if("".equals(txtOutputsAddName.getText()) || exist) btnOutputsAdd.setEnabled(false);
                else                                                btnOutputsAdd.setEnabled(true);
            }});
        ...
        btnOutputsRemove.addSelectionListener(new SelectionAdapter() {
            @Override public void widgetSelected(SelectionEvent event) {
                ...
                if(selectionIds.length != 0) btnOutputsRemove.setEnabled(false);
                for (int i = 0, n = selectionIds.length; i < n; i++)
                    tblOutputs.remove(selectionIds[i]);
            }});
        tblParameters.addSelectionListener(new SelectionAdapter() {
            @Override public void widgetSelected(SelectionEvent event) { btnParametersRemove.setEnabled(true); }
        });
        txtParametersAddName.addModifyListener(new ModifyListener() {
            @Override public void modifyText(ModifyEvent event) {
                boolean exist = false;
                for(TableItem it : tblParameters.getItems())
                    if(it.getText(0).equals(txtParametersAddName.getText())) exist = true;
                if("".equals(txtParametersAddName.getText()) || exist) btnParametersAdd.setEnabled(false);
                else                                                   btnParametersAdd.setEnabled(true);
            }});
        btnParametersAdd.addSelectionListener(new SelectionAdapter() {
            @Override public void widgetSelected(SelectionEvent event) {
                TableItem item = new TableItem(tblParameters, SWT.NONE);
                item.setText(new String[] { txtParametersAddName.getText(), txtParametersAddValue.getText() });
                for(TableColumn tc : tblParameters.getColumns()) tc.pack();
                sortParameters();
        ...
        btnParametersRemove.addSelectionListener(new SelectionAdapter() {
            @Override public void widgetSelected(SelectionEvent event) {
                ...
                if(selectionIds.length != 0) btnParametersRemove.setEnabled(false);
                for (int i = 0, n = selectionIds.length; i < n; i++)
    ...
    public Module getModel() {
        ...
        for(TableItem it : tblInputs.getItems()) {
            ...
            in.setQueue(it.getText(5).equals(Constants.INF) ? -1 : Integer.parseInt(it.getText(5)));
        ...
        for(In in : module.getInputs())
            if(in.isAsynchronous()) hasAsync = true;

        boolean hasNonAsync = false;
        for(In in : module.getInputs())
            if(!in.isAsynchronous()) hasNonAsync = true;
