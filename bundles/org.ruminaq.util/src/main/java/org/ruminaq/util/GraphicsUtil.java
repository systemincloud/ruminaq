...
public class GraphicsUtil {
	...
	public static void alignWithShape( AbstractText text,
		...
		gaService.setLocationAndSize(labelContainer.getGraphicsAlgorithm(), newShapeX, newShapeY, textWidth + SHAPE_PADDING, textHeight + SHAPE_PADDING);
		gaService.setLocationAndSize(text, 0, 0, textWidth + TEXT_PADDING, textHeight + TEXT_PADDING);
	}

	public static boolean isLabelInDefaultPosition(ContainerShape label, int width, int height, int x, int y) {
		...
		if(almostEqual(currentLabelX, newShapeX, 4) && almostEqual(currentLabelY, newShapeY, 4)) return true;
	...
	public static void onTopOfShape(  AbstractText text,
		...
		gaService.setLocationAndSize(labelContainer.getGraphicsAlgorithm(), newShapeX, newShapeY, textHeight + SHAPE_PADDING, textWidth + SHAPE_PADDING);
		gaService.setLocationAndSize(text, -(textWidth >> 1) + TEXT_PADDING, -3, textWidth + SHAPE_PADDING, textWidth + SHAPE_PADDING);
		text.setHorizontalAlignment(Orientation.ALIGNMENT_CENTER);
		text.setVerticalAlignment(Orientation.ALIGNMENT_CENTER);
	}
	...
	public static void onBottomOfShape(AbstractText text,
		...
		gaService.setLocationAndSize(labelContainer.getGraphicsAlgorithm(), newShapeX, newShapeY, textHeight, textWidth + SHAPE_PADDING);
		gaService.setLocationAndSize(text, -(textWidth >> 1) + TEXT_PADDING, TEXT_PADDING, textWidth + TEXT_PADDING, textWidth);
		text.setVerticalAlignment(Orientation.ALIGNMENT_CENTER);
	...
	public static boolean intersectsLabel(ContainerShape label, Shape shape) {
		...
		xal = label.getGraphicsAlgorithm().getX();              yal = label.getGraphicsAlgorithm().getY();
		xbl = xal;                              		        ybl = yal + label.getGraphicsAlgorithm().getHeight();
		xdl = xal + label.getGraphicsAlgorithm().getWidth();    ydl = yal;
		xcl = xdl;                                              ycl = ybl;

		xas = shape.getGraphicsAlgorithm().getX();              yas = shape.getGraphicsAlgorithm().getY();
		xbs = xas;                                              ybs = yas + shape.getGraphicsAlgorithm().getHeight();
		xds = xas + shape.getGraphicsAlgorithm().getWidth();    yds = yas;
		xcs = xds;                                              ycs = ybs;

		if((xcl > xas && ycl > yas) && (xcl < xcs && ycl < ycs)) return true;
		if((xdl > xbs && ydl < ybs) && (xdl < xds && ydl > yds)) return true;
		if((xal < xcs && yal < ycs) && (xal > xas && yal > yas)) return true;
		if((xbl < xds && ybl > yds) && (xbl > xbs && ybl < ybs)) return true;
	...
	public static int distanceToConnection(FreeFormConnection ffc, int x, int y, String internalPortProperty) {
		...
		String isInternalPort = Graphiti.getPeService().getPropertyValue(ffc.getStart().getParent(), internalPortProperty);
		if(Boolean.parseBoolean(isInternalPort)) {
			x_start = x_start + ((ContainerShape) ffc.getStart().getParent()).getContainer().getGraphicsAlgorithm().getX();
			y_start = y_start + ((ContainerShape) ffc.getStart().getParent()).getContainer().getGraphicsAlgorithm().getY();
		}
		int x_end   = ffc.getEnd()  .getParent().getGraphicsAlgorithm().getX() + (ffc.getEnd()  .getParent().getGraphicsAlgorithm().getWidth()  >> 1);
		int y_end   = ffc.getEnd()  .getParent().getGraphicsAlgorithm().getY() + (ffc.getEnd()  .getParent().getGraphicsAlgorithm().getHeight() >> 1);
		isInternalPort = Graphiti.getPeService().getPropertyValue(ffc.getEnd().getParent(), internalPortProperty);
		if(Boolean.parseBoolean(isInternalPort)) {
			x_end = x_end + ((ContainerShape) ffc.getEnd().getParent()).getContainer().getGraphicsAlgorithm().getX();
			y_end = y_end + ((ContainerShape) ffc.getEnd().getParent()).getContainer().getGraphicsAlgorithm().getY();
		}
		...
		for(int i = 0; i <= points.size(); i++) {
			...
			if      (i == 0)                     { x_before = x_start;                  y_before = y_start;                  x_next = points.get(i).getX(); y_next = points.get(i).getY(); }
			else if (i > 0 && i < points.size()) { x_before = points.get(i - 1).getX(); y_before = points.get(i - 1).getY(); x_next = points.get(i).getX(); y_next = points.get(i).getY(); }
			else 				                 { x_before = points.get(i - 1).getX(); y_before = points.get(i - 1).getY(); x_next = x_end;                y_next = y_end;                }
	...
	public static int distanceToSection(int x1, int y1, int x2, int y2, int xp, int yp) {
		Point d = projectionOnSection(x1, y1, x2, y2, xp, yp);
		if(((Math.min(d.getX(), x1) <= x2) && (x2 <= Math.max(d.getX(), x1)) && (Math.min(d.getY(), y1) <= y2) && (y2 <= Math.max(d.getY(), y1)))) return Integer.MAX_VALUE;
		return (int) Math.sqrt(Math.pow(d.getX() - xp, 2) + Math.pow(d.getY() - yp, 2));
	}

	private static Point projectionOnSection(int x1, int y1, int x2, int y2, int xp, int yp) {
		int denominator = x1*x1 - 2*x1*x2 + x2*x2 + y1*y1 - 2*y1*y2 + y2*y2;
		...
		if(denominator == 0) {
			...
			d_x = (xp*x1*x1 - 2*xp*x1*x2 - x1*y1*y2 + yp*x1*y1 + x1*y2*y2 - yp*x1*y2 + xp*x2*x2 + x2*y1*y1 - x2*y1*y2 -   yp*x2*y1 + yp*x2*y2) / denominator;
			d_y = (x1*x1*y2 -   x1*x2*y1 - x1*x2*y2 + xp*x1*y1 - xp*x1*y2 + x2*x2*y1 - xp*x2*y1 + xp*x2*y2 + yp*y1*y1 - 2*yp*y1*y2 + yp*y2*y2) / denominator;
		}
		return createPoint(d_x, d_y);
	}

	public static boolean pointBelongsToSection(Point d, int x1, int y1, int x2, int y2) {
		int det = d.getX()*y1 + x1*y2 + x2*d.getY() - x2*y1 - d.getX()*y2 - x1*d.getY();
		if (det!=0) return false;
		else if ((Math.min(d.getX(), x1) <= x2) && (x2 <= Math.max(d.getX(), x1)) && (Math.min(d.getY(), y1) <= y2) && (y2 <= Math.max(d.getY(), y1))) return true;
		else return false;
	}

	public static boolean pointBelongsToSection(Point p, int x1, int y1, int x2, int y2, int epsilon) {
		int d = distanceToSection(x1, y1, x2, y2, p.getX(), p.getY());
		if(d <= epsilon) return true;
		else return false;
	}

	public static Point projectOnConnection(FreeFormConnection ffc, int x, int y, String internalPortProperty) {
		...
		if(points.size() == 0) {
			...
			if(((Math.min(pd.getX(), x_start) <= x_end) && (x_end <= Math.max(pd.getX(), x_start)) && (Math.min(pd.getY(), y_start) <= y_end) && (y_end <= Math.max(pd.getY(), y_start)))) return null;
			else return pd;
		}
		...
		for(int i = 0; i <= points.size(); i++) {
			...
			if(((Math.min(pd.getX(), x_before) <= x_next) && (x_next <= Math.max(pd.getX(), x_before)) && (Math.min(pd.getY(), y_before) <= y_next) && (y_next <= Math.max(pd.getY(), y_before)))) continue;
			int d_tmp = (int) Math.sqrt(Math.pow(pd.getX() - x, 2) + Math.pow(pd.getY() - y, 2));
			if(d_tmp < d) {
				d = d_tmp;
				p = pd;
			}
	...
	public static Polygon createArrow(GraphicsAlgorithmContainer gaContainer, Diagram diagram) {
