...
public class GlobalUtil {
	...
	/*
	 * Dimensions are integers separated by ','
	 * Example: '4, 4   , 3, 4,5'
	 */
	public static boolean isDimensions(String value) {
		if(value.matches(".*[0-9]\\s+[0-9].*")) return false;
		String tmp = value.replaceAll("\\s+", "");
		if (tmp.matches(".*,,.*"))  return false;
		if (tmp.matches(",.*"))     return false;
		if (tmp.matches("[0-9,]+")) return true;
		else                       return false;
	}
	...
	public static boolean isTimeAlsoGVandRand(String time) {
		if(GlobalUtil.isGlobalVariable(time)) return true;
	...
	public static boolean isTime(String time) {
		return time.matches("^((\\d+([.,]\\d+)?\\s*(ms|s|m|h|d)\\s*)+)|(\\s*inf\\s*)$");
	}
	...
	public static long getMilisecondsFromTime(String time) {
		if(!GlobalUtil.isTime(time)) throw new ParseError("Can't understand written internval.");

		if(time.matches("\\s*inf\\s*")) return -1;
		...
		for(Tokenizer.Token tok : tokenizer.getTokens()) {
			...
			if(digit > 0) tmp = new StringBuilder(tmp).insert(digit, ".").toString();
			float f = Float.valueOf(tmp);
			switch(tok.token) {
				case 1: ret += f; break;
				case 2: ret += f*1000; break;
				case 3: ret += f*60*1000; break;
				case 4: ret += f*60*60*1000; break;
				case 5: ret += f*24*60*60*1000; break;
			}
		}
		return ret;
	}
	...
	public static List<Integer> getDimensions(String string) {
		...
		for(String d : dims)
			if(!"".equals(d)) ret.add(Integer.parseInt(d));
		return ret;
	}
}
