...
public class InternalPortBreakpointEditor extends AbstractJavaBreakpointEditor {
  ...
  enum Properties {
    PROP_HIT_COUNT_ENABLED, PROP_HIT_COUNT, PROP_SUSPEND_POLICY
  }
  ...
  private void initLayout(Composite parent) {
    ...
    btnHitCount = SWTFactory.createCheckButton(root,
        processMnemonics("Hit count:"), null, false, 1);
    ...
    radios = SWTFactory.createComposite(root, root.getFont(), 2, 1,
        GridData.FILL_HORIZONTAL, 0, 0);
    btnSuspendPort = SWTFactory.createRadioButton(radios,
        processMnemonics("Suspend Port"), 1);
    btnSuspendPort.setLayoutData(new GridData());
    btnSuspendAll = SWTFactory.createRadioButton(radios,
        processMnemonics("Suspend Runner"), 1);
  ...
  public void setInput(Object breakpoint) throws CoreException {
    try {
      ...
      if (breakpoint instanceof InternalPortBreakpoint)
        setBreakpoint((InternalPortBreakpoint) breakpoint);
      else
        setBreakpoint(null);
    } finally {
  ...
  protected void setBreakpoint(InternalPortBreakpoint bp) throws CoreException {
    ...
    if (breakpoint != null) {
      ...
      suspendPort = breakpoint
          .getSuspendPolicy() == InternalPortBreakpoint.SuspendPolicy.SUSPEND_PORT;
    }
  ...
  protected InternalPortBreakpoint getBreakpoint() {
    return breakpoint;
  }
  ...
  public void doSave() throws CoreException {
    if (breakpoint != null) {
      ...
      if (btnSuspendAll.getSelection())
        suspendPolicy = InternalPortBreakpoint.SuspendPolicy.SUSPEND_RUNNER;
      breakpoint.setSuspendPolicy(suspendPolicy);
      int hitCount = -1;
      if (btnHitCount.getSelection()) {
        try {
          hitCount = Integer.parseInt(txtHitCountText.getText());
        } catch (NumberFormatException e) {
          throw new CoreException(
              new Status(IStatus.ERROR, JDIDebugUIPlugin.getUniqueIdentifier(),
                  IStatus.ERROR, HIT_COUNT_NOT_VALID_MSG, e));
        }
      }
  ...
  public IStatus getStatus() {
    if (btnHitCount.getSelection()) {
      ...
      try {
        ...
        return new Status(IStatus.ERROR, JDIDebugUIPlugin.getUniqueIdentifier(),
            IStatus.ERROR, HIT_COUNT_NOT_VALID_MSG, null);
      }
      if (hitCount < 1) {
        return new Status(IStatus.ERROR, JDIDebugUIPlugin.getUniqueIdentifier(),
            IStatus.ERROR, HIT_COUNT_NOT_VALID_MSG, null);
  ...
  protected Button createSusupendPropertyEditor(Composite parent, String text,
      final int propId) {
