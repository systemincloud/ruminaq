...
public final class NewTaskEvent extends AbstractEvent
  ...
  public List<NewInputPort> getInputPorts() {
    return inPorts;
  }

  public List<NewOutputPort> getOutputPorts() {
    return outPorts;
  }
  ...
  public void preevaluate() {
    ...
    this.fullId += this.id;
    this.fullId += " ["
        + taskI.getModel().getClass().getInterfaces()[0].getSimpleName() + "]";

    for (InternalInputPortI p : taskI.getInternalInputPorts())
      inPorts.add(new NewInputPort(p.getPortId()));
    for (InternalOutputPortI p : taskI.getInternalOutputPorts())
      outPorts.add(new NewOutputPort(p.getPortId()));
  }
}
