...
public class RawI extends DataI {
  ...
  public DataI toDecimalI() {
    ...
    for (byte[] v : this.values) {
      ...
      int scale = ByteBuffer.wrap(scaleBytes).order(ByteOrder.BIG_ENDIAN)
          .asIntBuffer().array()[0];
      byte[] lengthBytes = Arrays.copyOfRange(v, 4, 7);
      int length = ByteBuffer.wrap(lengthBytes).order(ByteOrder.BIG_ENDIAN)
          .asIntBuffer().array()[0];
  ...
  public DataI toInt32I() {
    IntBuffer buf = ByteBuffer.wrap(this.values.get(0))
  ...
  public DataI toInt64I() {
    LongBuffer buf = ByteBuffer.wrap(this.values.get(0))
  ...
  public DataI toTextI() {
    ...
    for (byte[] v : this.values) {
      ...
      IntBuffer intBuffer = ByteBuffer.wrap(lengthBytes)
          .order(ByteOrder.BIG_ENDIAN).asIntBuffer();
