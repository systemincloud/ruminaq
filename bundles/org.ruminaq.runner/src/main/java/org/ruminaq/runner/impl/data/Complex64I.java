...
public class Complex64I extends NumericI {
    ...
    private Complex64I() { super(new LinkedList<Integer>()); }
    ...
    public Complex64I(double[] real, double[] imag, List<Integer> dims, boolean copy) {
        super(dims);
        if(copy) {
    ...
    public double[] getRealValues() { return real; }
    public double[] getImagValues() { return imag; }
    ...
    @Override protected DataI to(Class<? extends DataI> clazz) {
        ...
        else if(Complex64I.class.isAssignableFrom(clazz)) return this;
    ...
    public DataI toTextI() {
        ...
        for(int i = 0; i < nElements; i++) {
            ...
            values[i] = Double.toString(this.real[i]) + sign +  Double.toString(Math.abs(this.imag[i])) + "i";
        }
