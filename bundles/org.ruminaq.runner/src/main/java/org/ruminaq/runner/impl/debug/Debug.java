...
public class Debug implements DebugVisited {
	...
	public void hit() { if(breakpoint && hits < hitCount) hits++; }

	public boolean resume(Type type) {
		lock.lock();
		logger.trace("resume");
		if(type == Type.STEP_OVER) this.stepping = true;
	...
	public void trySuspend() {
		...
		if(suspended || stepping || ((hits >= hitCount) && breakpoint)) {
			...
			if(stepping)   listener.steppingSuspend();
			if(breakpoint) listener.breakpointSuspend();
			if(suspendAll) listener.suspendAll();
			try { condition.await();
	...
	public void breakpoint(boolean b, int hitCount, boolean suspendAll) {
		logger.trace("breakpoint: {}, hit count: {}, suspendAll: {}", b, hitCount, suspendAll);
		this.breakpoint = b;
