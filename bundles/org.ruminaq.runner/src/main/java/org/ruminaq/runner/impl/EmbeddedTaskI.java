...
public class EmbeddedTaskI extends TaskI {
    ...
    private final Logger logger = RunnerLoggerFactory.getLogger(EmbeddedTaskI.class);

    private List<TaskI>      tasks       = new LinkedList<TaskI>();      //Collections.synchronizedList(new LinkedList<TaskI>());
    private List<BasicTaskI> constants   = new LinkedList<BasicTaskI>(); //Collections.synchronizedList(new LinkedList<BasicTaskI>());
    private List<GeneratorI> generators  = new LinkedList<GeneratorI>(); //Collections.synchronizedList(new LinkedList<GeneratorI>());
    ...
    public  boolean hasExternalSource() { return hasExternalSource; }
    private void    setHasExternalSourceParents(boolean b) {
        this.hasExternalSource = b;
        if(parent != null) parent.setHasExternalSourceParents(b);
    }

    public boolean getSyncConns() { return syncConns; }
    ...
    public void executeConstants() {
        ...
        for(BasicTaskI c : constants) c.executeConstant();
        for(TaskI t : tasks)
...
//	public void hangEngine()   { parent.hangEngine(); }
    ...
    public LinkedList<TaskI> getReadyTasks() {
        ...
        for(TaskI task : tasks)
            if(task.isReady()) {
                logger.trace("{}:{}: is ready", task.getClass().getSimpleName(), task.getId());
...
//				hangOnHangingConsts();
    ...
    private void addGenerator (GeneratorI impl) { generators.add(impl); }
    private void addConstant  (BasicTaskI impl) { constants.add(impl); }
    private void addTask      (TaskI impl)      {
        tasks.add(impl);
        if(impl.isExternalSoure()) setHasExternalSourceParents(true);
    }
    ...
    public List<? extends GeneratorI> getAllGenerators() {
        ...
        for(TaskI t : tasks) allGenerators.addAll(t.getAllGenerators());
...
//	}
    ...
    private void createImplementation(MainTask mainTask) {
        ...
        for(Task task : mainTask.getTask())  {
            if(isOnlyLocal(task) && inCloud) continue;
            if(isOnlyLocal(task) && !runOnlyLocal && !ConstantsUtil.isTest(task.eResource().getURI(), getBasePath())) continue;

            TaskI taskI = TaskImplementationFactory.INSTANCE.getImplementation(this, task, getBasePath(), inCloud, runOnlyLocal);
            if(taskI == null) throw new RunnerException("No implementation found for task: " + task.getId());
            ...
            for(InternalInputPortI  iip : taskI.getInternalInputPorts())  { inputsTmp .put(iip.getModel(), iip); }
            for(InternalOutputPortI iop : taskI.getInternalOutputPorts()) { outputsTmp.put(iop.getModel(), iop); }
        }
        logger.trace("EmbeddedTask has {} task(s), {} generator(s), "
                                    + "{} input(s), {} output(s)", tasks.size(), generators.size(),
                                                                   internalInputPorts.size(), internalOutputPorts.size());
        // connections
            ...
            if(target == null && ft instanceof OutputPort) {
                for(InternalOutputPortI iop : internalOutputPorts.values())
                    if(iop.getPortId().equals(((OutputPort) ft).getId())) {
                        target = iop;
                        logger.trace("... to {}:{}", ((InternalOutputPortI) target).getTaskId(), ((InternalOutputPortI) target).getPortId());
                        break;
                    }
            } else if(target != null) logger.trace("... to {}:{}", ((InternalInputPortI) target).getTaskId(), ((InternalInputPortI) target).getPortId());
            if(target == null) continue;

            FlowSource st = c.getSourceRef();
            Observable source = outputsTmp.get(st);
            if(source != null) {
                logger.trace("... from {}:{}", ((InternalOutputPortI) source).getTaskId(), ((InternalOutputPortI) source).getPortId());
                source.addObserver(target);
            } else if(st instanceof InputPort) {
                for(InternalInputPortI iip : internalInputPorts.values())
                    if(iip.getPortId().equals(((InputPort) c.getSourceRef()).getId())) { source = iip; break; }
                if(source != null) {
                    logger.trace("... from {}:{}", ((InternalInputPortI) source).getTaskId(), ((InternalInputPortI) source).getPortId());
                    source.addObserver(target);
                }
            }
        }
        ...
        // synchronization
        for(TaskI t : tasks)      synchronizationInit(t);
        for(TaskI t : constants)  synchronizationInit(t);
        for(TaskI t : generators) synchronizationInit(t);
    }

    private boolean isOnlyLocal(Task task) { return task.isOnlyLocalDefault() ? task.isOnlyLocal() : task.isOnlyLocalUser(); }
                ...
                for(SynchronizationI s : o.getSyncs()) {
                    ...
                    if(p1 != null) {
                        ...
                        if(p1 instanceof InternalInputPort) {
                            ...
                            InternalOutputPortI op = getTask(taskId).getOutputPort(p1.getId());
        ...
        if(!isAtomic() || input.isAsynchronous()) {
            ...
            for(InternalInputPortI iip : internalInputPorts.values()) {
                if(!iip.isAsynchronous()) {
                    if(!grps.containsKey(iip.getGroup())) grps.put(iip.getGroup(), true);
                    if(iip.hasData()) grps.put(iip.getGroup(), true && grps.get(iip.getGroup()));
                    else              grps.put(iip.getGroup(), false);
                }
            }
            for(Entry<Integer, Boolean> grp : grps.entrySet())
                if(grp.getValue()) {
                    ...
                    for(InternalInputPortI iip : internalInputPorts.values()) {
                        if(iip.isAsynchronous()) continue;
                        if(iip.getGroup() != grp.getKey().intValue()) continue;
    ...
    public String replaceVariables(String string) {
        ...
        while(m.find()) {
            int start = m.start();
            if(start != lastIdx) sb.append(ret.substring(lastIdx, start));
            lastIdx      = m.end();
            String var   = m.group();
    ...
    public String getParameter(String key) {
        if(!parameters.containsKey(key)) {
            System.err.println("No parameter " + key + " defined in task " + this.getId());
