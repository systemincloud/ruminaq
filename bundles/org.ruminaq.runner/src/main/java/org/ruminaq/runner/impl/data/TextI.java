...
public class TextI extends DataI {
  ...
  public RemoteData getRemoteData() {
    ...
    for (String v : this.values) {
      try {
        byteStream
            .write(ByteBuffer.allocate(4).putInt(v.getBytes().length).array());
  ...
  public String toString(boolean isPretty, int limit, Separator separator) {
    ...
    if (this.getDimensions().size() == 1 && this.getDimensions().get(0) == 1)
      value.append(this.getValues().get(0));
    else if (this.getDimensions().size() == 1
        && this.getDimensions().get(0) != 1) {
      value.append("[");
      ...
      value.append("[");
      if (isPretty)
        value.append(separator.getChars());
      ...
      for (String s : this.getValues()) {
        ...
        i++;
        if (isPretty)
          value.append(separator.getChars() + separator.getChars());
        else
          value.append(", ");
        if (i % n == 0) {
          if (isPretty)
            ...
            value.delete(value.length() - 2, value.length() - 1).append("; ");
      ...
      for (int i = 0; i < this.getDimensions().size() - 2; i++)
        ids.add(0);
      ...
      for (String s : this.getValues()) {
        ...
        if (i % mn == 0) {
          ...
          for (int x = 0; x < ids.size(); x++) {
            if (ids.get(x) + 1 == this.getDimensions().get(x + 2))
              ids.set(x, 0);
            else {
              ids.set(x, ids.get(x) + 1);
              break;
            }
          }
        }
  ...
  public DataI toComplex32I() {
    ...
    for (int i = 0; i < nElements; i++) {
      ...
      else if (s.matches("[-]?i[0-9]*\\.?[0-9]+"))
        imag[i] = Float.parseFloat(s);
      else {
        int k = s.contains("+") ? s.indexOf("+") : s.indexOf("-");
        real[i] = Float.parseFloat(s.substring(0, k).trim());
        imag[i] = Float
            .parseFloat(s.substring(s.indexOf("i"), s.length()).trim());
      }
    }
  ...
  public DataI toComplex64I() {
    ...
    for (int i = 0; i < nElements; i++) {
      ...
      if (s.matches("[-]?[0-9]*\\.?[0-9]+"))
        ...
        imag[i] = Double.parseDouble(s);
      else {
        int k = s.contains("+") ? s.indexOf("+") : s.indexOf("-");
        real[i] = Double.parseDouble(s.substring(0, k).trim());
        imag[i] = Double
            .parseDouble(s.substring(s.indexOf("i"), s.length()).trim());
      }
    }
