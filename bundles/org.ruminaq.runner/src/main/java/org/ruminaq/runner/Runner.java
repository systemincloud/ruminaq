...
import org.ruminaq.model.model.dt.DatatypePackage;
...
public class Runner {
    ...
    private void init(String testFile, boolean onlyLocal, CommandLine line) {
        System.out.println("Running: " + testFile.replace("/", File.separator).replace("\\", File.separator));
        ...
        this.basePath = testFile.substring(0, testFile.indexOf(SourceFolders.TEST_RESOURCES));
        try {
    ...
    public static void main(String[] args) {
        ...
        String projectRoot = "";
        if(line.hasOption(ATTR_ROOT)) projectRoot = line.getOptionValue(ATTR_ROOT).replace("+", " ");
        else {
        ...
        String  testFile;
        boolean onlyLocal;
        if(line.hasOption(ATTR_DIAGRAM)) testFile   = line.getOptionValue(ATTR_DIAGRAM).replace("+", " "); else return;
