...
public abstract class TaskI implements Callable<ExecutionReport> {
  ...
  public EmbeddedTaskI getParent() {
    return parent;
  }
  ...
  public InternalInputPortI getInputPort(String id) {
    return internalInputPorts.get(id);
  }

  public InternalOutputPortI getOutputPort(String id) {
    return internalOutputPorts.get(id);
  }
  ...
  public Lock getOutputsLock() {
    return lockOutputs;
  }

  public Lock getInputsLock() {
    return lockInputs;
  }

  public Condition getOutputsSync() {
    return outputsSync;
  }

  private Set<Integer> syncGrps;

  public void setSyncGroups(Set<Integer> syncGrps) {
    this.syncGrps = syncGrps;
  }

  private Map<InternalOutputPortI, Boolean> toSend = Collections
      .synchronizedMap(new HashMap<InternalOutputPortI, Boolean>());
  ...
  public void putDataSync(InternalOutputPortI iop) {
    if (toSend.get(iop) == null)
      toSend.put(iop, false);
  }
  ...
  protected void send() {
    ...
    if (p0s != null) {
      logger.trace("{}:{}: check group 0", this.getClass().getSimpleName(),
          this.getId());
    ...
    for (Integer i : syncGrps) {
      logger.trace("{}:{}: check group {}", this.getClass().getSimpleName(),
          this.getId(), i.intValue());
      ...
      for (InternalOutputPortI p : ps) {
        ...
        if (b == null)
          return;
        if (b == false && p.getCurrentSync().getSyncPort() != null)
          return;
      }
      // send
      logger.trace("{}:{}: send group {}", this.getClass().getSimpleName(),
          this.getId(), i.intValue());
  ...
  protected boolean isWaitingForSync() {
    for (InternalOutputPortI p : internalOutputPorts.values())
      if (p.isWaiting()) {
        logger.trace("{}:{}:{} is waiting", this.getClass().getSimpleName(),
            this.getId(), p.getPortId());
        return true;
      }
    return false;
  }
  ...
  public TaskI(EmbeddedTaskI parent, Task task) {
    ...
    logger.trace("{}:{}: atomic {}", this.getClass().getSimpleName(),
        this.getId(), this.atomic);
    logger.trace("{}:{}: looped {}", this.getClass().getSimpleName(),
        this.getId(), this.looped);

    for (InternalInputPort ip : task.getInputPort())
      internalInputPorts.put(ip.getId(), new InternalInputPortI(ip, this));
    for (InternalOutputPort op : task.getOutputPort())
      internalOutputPorts.put(op.getId(), new InternalOutputPortI(op, this));
  }
  ...
  public void setReadyWithParents(boolean ready) {
    ...
    setReady(ready);
    parent.setReadyWithParents(ready);
...
//	}
  ...
  public boolean isExternalSoure() {
  ...
  /**
   * Implementation of task can mark in its constructor that is of type
   * 'constant'
   *
   * @param value
   */
  ...
  public void modelEvent(IDebugEvent event) {
    for (InternalInputPortI ip : internalInputPorts.values())
      ip.modelEvent(event);
    for (InternalOutputPortI ip : internalOutputPorts.values())
      ip.modelEvent(event);
  }

  public void spreadDebugEvent(IDebugEvent event) {
    parent.spreadDebugEvent(event);
  }
