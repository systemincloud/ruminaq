...
public enum DirmiClient {
	...
	private final Logger logger = RunnerLoggerFactory.getLogger(DirmiClient.class);
	...
	public void init(int mainPort) {
		...
		try {
			Session mainSession = env.newSessionConnector("localhost", mainPort).connect();
	...
	public void register(String key, String name, Remote object) {
		logger.trace("key: {}, name: {}, object: {}", key, name, object.getClass().getSimpleName());
		...
		if(ls != null) {
			try { ls.registerRemote(name, object);
