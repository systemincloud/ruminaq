...
public abstract class BasicTaskI extends TaskI {
    ...
    private List<TaskExecutor> executors     = Collections.synchronizedList(new LinkedList<TaskExecutor>());
    private Lock               executorsLock = new ReentrantLock();

    protected void addExternalSrcExecNb(int externalSrcExecNb) {
        executorsLock.lock();
        while(externalSrcExecNb-- > 0) executors.add(new ExtTaskExecutor());
        executorsLock.unlock();
    }

    @Override
    public List<BasicTaskI> getReadyTasks() { return Arrays.asList(BasicTaskI.this); }
    ...
    protected interface TaskExecutor { void execute(); }
    ...
    private class AsyncTaskExecutor implements TaskExecutor {
        ...
        @Override
        public void execute() {
            logger.trace("{}:{}: async", this.getClass().getSimpleName(), BasicTaskI.this.getId());
            executeAsync(iip.getPortId(), iip.getData());
            logger.trace("{}:{}: async finished", this.getClass().getSimpleName(), BasicTaskI.this.getId());
        }
    }
    ...
    private class SyncTaskExecutor implements TaskExecutor {
        ...
        @Override
        public void execute() {
            logger.trace("{}:{}: sync", this.getClass().getSimpleName(), BasicTaskI.this.getId());
            ...
            logger.trace("{}:{}: sync finished", this.getClass().getSimpleName(), BasicTaskI.this.getId());
        }
    }

    private class ExtTaskExecutor implements TaskExecutor {
        @Override public void execute() {
            logger.trace("{}:{}: external src", this.getClass().getSimpleName(), BasicTaskI.this.getId());
            executeExternalSrc();
            logger.trace("{}:{}: external src finished", this.getClass().getSimpleName(), BasicTaskI.this.getId());
        }
    }
    ...
    public ExecutionReport call() {
        try {
            logger.debug("{}:{}: execute", this.getClass().getSimpleName(), this.getId());
            if(Thread.interrupted()) return new ExecutionReport(false, this);
            ...
            if(executors.size() > 0) {
                ...
                if(ex == null) ex = executors.remove(0);
                else                executors.remove(ex);
                executorsLock.unlock();
    ...
    protected void waitForSync() {
        ...
        while(isWaitingForSync()) {
            logger.trace("{}:{}: waiting for synchronization to be done", this.getClass().getSimpleName(), this.getId());
            try {
                outputsSync.await();
            } catch (InterruptedException e) {
                logger.trace("{}:{}: InterruptedException", this.getClass().getSimpleName(), this.getId());
            }
        }
        resetSync();
        lockOutputs.unlock();
    ...
    public void putData(String id, DataI data) {
        logger.trace("{}:{}: put data on {}", this.getClass().getSimpleName(), this.getId(), id);
        if(isConstant()) data.setConstant(true);
    ...
    public void portManager(InternalInputPortI input) {
        if(!isAtomic() || input.isAsynchronous()) {
            ...
            logger.trace("{}:{}:{}: is synchronous", this.getClass().getSimpleName(), this.getId(), input.getPortId());
            if(checkSync(input.getGroup())) {
                executorsLock.lock();
                executors.add(new SyncTaskExecutor(input.getGroup()));
                executorsLock.unlock();
                logger.trace("{}:{}: set ready with parents", this.getClass().getSimpleName(), this.getId());
                setReadyWithParents(true);
            }
        }
    }
    ...
    private boolean checkSync(int grp) {
        ...
        for(InternalInputPortI iip : internalInputPorts.values()) {
            if(!iip.isAsynchronous() && iip.getGroup() == grp) {
                ...
                if(!holdLast) onlyHold = false;
                if(holdLast && !lastRead) hasHoldNotRead = true;

                logger.trace("{} hasData: {}, holdLast: {}, lastRead {}", iip.getPortId(), hasData, holdLast, lastRead);
            }
        }
        ...
        // hasHoldNotRead : there is at least one synchronous port that can hold last data and this data hasn't been read
        //
        logger.trace("sync {}, onlyHold {}, hasHoldNotRead {}", sync, onlyHold, hasHoldNotRead);
