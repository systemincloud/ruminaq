...
public enum DebugI {
	...
	public void init(boolean debugOn) {
		...
		DirmiClient.INSTANCE.register("DEBUG", "main", new IDebugIService() {
			@Override public void setDebugService(IDebugService ds) {
			    logger.trace("setDebugService");
			    DebugI.this.ds = ds;
			}
			@Override
			public void modelEvent(IDebugEvent event) throws RemoteException {
				if(event instanceof TerminateRequest) terminate();
				else engine.modelEvent(event);
			}
		});
	}

	public void setEngine(Engine engine) { this.engine = engine; }
	public void initDebugers()           { if(debugOn) engine.initDebugers(); }

	public void debug(IDebugEvent event) {
		if(!debugOn) return;
		event.preevaluate();
		try { ds.debuggerEvent(event);
