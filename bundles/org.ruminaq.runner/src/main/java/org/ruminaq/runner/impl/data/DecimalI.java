...
public class DecimalI extends NumericI {
    ...
    private DecimalI() { super(new LinkedList<Integer>()); }
    ...
    public DecimalI(RemoteData data) {
        ...
        for(String s : new String(data.getBuf()).split(" "))
            if(!"".equals(s)) this.values.add(new BigDecimal(s));
    }
    ...
    public RemoteData getRemoteData() {
        ...
        for(BigDecimal v : this.values) {
            ...
            try {
                ...
                if(missingbytes != 0) missingbytes = 4 - missingbytes;
                byteStream.write(ByteBuffer.allocate(4).putInt(bv.length + missingbytes).array());
    ...
    @Override protected DataI to(Class<? extends DataI> clazz) {
        ...
        else if(DecimalI  .class.isAssignableFrom(clazz)) return this;
    ...
    public class DecimalIRawWrap {
        ...
        public byte[] getBigInteger() { return bigInteger; }
    }
    ...
    public DataI toRawI() {
        ...
        for(DecimalIRawWrap w : wraps) {
            ...
            System.arraycopy(ByteBuffer.allocate(4).putInt(w.getScale()).array(),  0, value, 0 , 3);
            System.arraycopy(ByteBuffer.allocate(4).putInt(w.getLength()).array(), 0, value, 4 , 7);
