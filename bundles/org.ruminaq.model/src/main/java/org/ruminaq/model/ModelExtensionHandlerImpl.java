...
public class ModelExtensionHandlerImpl implements ModelExtensionHandler {
  ...
  public Collection<? extends Class<? extends DataType>> getDataTypes() {
    return extensions.stream().map(ModelExtension::getDataTypes)
        .flatMap(Collection::stream).collect(Collectors.toList());
  }

  @Override
  public Optional<DataType> getDataTypeFromName(String name) {
    return extensions.stream().map(ext -> ext.getDataTypeFromName(name))
        .filter(Optional::isPresent).findFirst().get();
  }

  @Override
  public boolean canCastFromTo(Class<? extends DataType> from,
      Class<? extends DataType> to) {
    return extensions.stream().anyMatch(ext -> ext.canCastFromTo(from, to));
  }

  @Override
  public boolean isLossyCastFromTo(Class<? extends DataType> from,
      Class<? extends DataType> to) {
    return extensions.stream().anyMatch(ext -> ext.isLossyCastFromTo(from, to));
  }
