...
public class JavaTaskI extends GeneratorI implements JavaTaskListener {
    ...
    public JavaTaskI(EmbeddedTaskI parent, Task task) {
        super(parent, task);
        String implementationClass = ((org.ruminaq.tasks.javatask.model.javatask.JavaTask) task).getImplementationClass();
        try {
            this.clazz = (Class<? extends JavaTask>) JavaTaskI.class.getClassLoader().loadClass(implementationClass);
        ...
        // Parameters
        //
        for(Entry<String, String> p : ((org.ruminaq.tasks.javatask.model.javatask.JavaTask) task).getParameters().entrySet())
                ...
                if(ipi != null) {
                    ...
                    try {
                        for(Field ipf : InputPort.class.getDeclaredFields())
                            if(ipf.getType().equals(JavaTaskListener.class)) { ipf.setAccessible(true); ipf.set(ip, this); }
                    ...
                    try {
                        for(Field opf : OutputPort.class.getDeclaredFields())
                            if(opf.getType().equals(JavaTaskListener.class)) { opf.setAccessible(true); opf.set(op, this); }
                    } catch (IllegalAccessException e) { }
                    f.setAccessible(true);
                    f.set(jt, op);
                }
    ...
    private InputPort getInputPortById(JavaTask jt, String portId) throws IllegalArgumentException, IllegalAccessException {
        for(Field f : clazz.getFields()) {
            InputPortInfo  ipi = f.getAnnotation(InputPortInfo.class);
            if(ipi != null) {
                if(portId.equals(ipi.name())) return (InputPort) f.get(jt);
            }
        }
        return null;
    }
    ...
    @Override public Object runExpression(String expression) { return GroovyExpressionUtil.evaluate(expression); }
    @Override public Logger log()                            { return jtLogger; }

    @Override public void runnerStart() { jt.runnerStart(); }
    @Override public void runnerStop()  { jt.runnerStop(); }

    @SuppressWarnings("unchecked")
    @Override public <T extends Data> T getData(InputPort ip, Class<T> type) {
        ...
        if(dataI != null) {
            ...
            if     (Data.class.equals(type))           return (T) JavaTaskDataConverter.INSTANCE.toJavaTaskData(dataI);
            else if(Data.class.isAssignableFrom(type)) return (T) JavaTaskDataConverter.INSTANCE.toJavaTaskData(dataI, type);
        }
        return null;
    }
    ...
    @Override public void putData(OutputPort op, Data data, boolean copy) {
        if(data == null) return;
