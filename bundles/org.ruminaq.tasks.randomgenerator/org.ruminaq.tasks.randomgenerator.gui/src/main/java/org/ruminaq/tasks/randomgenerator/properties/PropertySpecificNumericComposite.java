...
public class PropertySpecificNumericComposite
    extends PropertySpecificComposite {

  private CLabel lblDistribution;
  private Text txtDistribution;

  public PropertySpecificNumericComposite(ValueSaveListener saveListener,
  ...
  private void initLayout() {
    ...
    GridData layoutDist = new GridData(SWT.LEFT, SWT.CENTER, false, false, 1,
        1);
    layoutDist.minimumWidth = 75;
    layoutDist.widthHint = 75;
  ...
  private void initActions() {
    txtDistribution.addFocusListener(new FocusAdapter() {
      ...
      public void focusLost(FocusEvent event) {
        ...
        boolean parse = RandomUtil.isRandomAlsoGV(txtDistribution.getText())
            || checkIfValue(txtDistribution.getText());
        if (parse) {
          ModelUtil.runModelChange(new Runnable() {
            public void run() {
              ...
              if (bo instanceof RandomGenerator) {
                ...
                rg.getSpecific().put(
                    RandomGeneratorNumericStrategy.NUMERIC_DISTRIBUTION,
                    txtDistribution.getText());
              }
            }
  ...
  public void initValues(EMap<String, String> eMap) {
    ...
    if (nd == null)
      eMap.put(RandomGeneratorNumericStrategy.NUMERIC_DISTRIBUTION,
          RandomGeneratorNumericStrategy.DEFAULT_DISTRIBUTION);
  }

  @Override
  public void refresh(final EMap<String, String> eMap) {
    String tmp = eMap.get(RandomGeneratorNumericStrategy.NUMERIC_DISTRIBUTION);
    txtDistribution.setText(tmp != null
        ? eMap.get(RandomGeneratorNumericStrategy.NUMERIC_DISTRIBUTION)
        : RandomGeneratorNumericStrategy.DEFAULT_DISTRIBUTION);
  }
}
