...
public class PropertySpecificComplexComposite extends PropertySpecificComposite {
	...
	public PropertySpecificComplexComposite(ValueSaveListener saveListener,	Composite specificRoot, PictogramElement pe, TransactionalEditingDomain ed) {
	...
	private void initLayout() {
		...
		btnRect.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false, false, 2, 1));
		btnPolar  = new Button(composite, SWT.RADIO);
		btnPolar.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false, false, 2, 1));

		lblCommonA  = new CLabel(composite, SWT.NONE);
		txtCommonA = new Text  (composite, SWT.BORDER);
		GridData lytCommonA = new GridData(SWT.LEFT, SWT.CENTER, false, false, 1, 1);
		lytCommonA.minimumWidth = 75; lytCommonA.widthHint = 75;
		txtCommonA.setLayoutData(lytCommonA);

		lblPolarMag = new CLabel(composite, SWT.NONE);
		txtPolarMag = new Text  (composite, SWT.BORDER);
		GridData lytPolarMag = new GridData(SWT.LEFT, SWT.CENTER, false, false, 1, 1);
		lytPolarMag.minimumWidth = 75; lytPolarMag.widthHint = 75;
		txtPolarMag.setLayoutData(lytPolarMag);

		lblCommonB  = new CLabel(composite, SWT.NONE);
		txtCommonB = new Text  (composite, SWT.BORDER);
		GridData lytCommonB = new GridData(SWT.LEFT, SWT.CENTER, false, false, 1, 1);
		lytCommonB.minimumWidth = 75; lytCommonB.widthHint = 75;
		txtCommonB.setLayoutData(lytCommonB);

		lblPolarArg = new CLabel(composite, SWT.NONE);
		txtPolarArg = new Text  (composite, SWT.BORDER);
		GridData lytPolarArg = new GridData(SWT.LEFT, SWT.CENTER, false, false, 1, 1);
		lytPolarArg.minimumWidth = 75; lytPolarArg.widthHint = 75;
	...
	private void initActions() {
		...
		txtCommonA.addFocusListener(new FocusAdapter() {
			@Override public void focusLost(FocusEvent event) {
				...
				boolean parse = RandomUtil.isRandomAlsoGV(txtCommonA.getText()) || NumericUtil.isOneDimNumeric(txtCommonA.getText());
				if(parse) {
					ModelUtil.runModelChange(new Runnable() {
						public void run() {
							Object bo = Graphiti.getLinkService().getBusinessObjectForLinkedPictogramElement(pe);
				...
				} else MessageDialog.openError(shell, "Can't edit value", "Don't understant distribution");
			}
		});
		...
		txtCommonB.addFocusListener(new FocusAdapter() {
			@Override public void focusLost(FocusEvent event) {
				...
				boolean parse = RandomUtil.isRandomAlsoGV(txtCommonB.getText()) || NumericUtil.isOneDimNumeric(txtCommonB.getText());
				if(parse) {
					ModelUtil.runModelChange(new Runnable() {
						public void run() {
							Object bo = Graphiti.getLinkService().getBusinessObjectForLinkedPictogramElement(pe);
		...
		txtPolarMag.addFocusListener(new FocusAdapter() {
			@Override public void focusLost(FocusEvent event) {
				...
				boolean parse = RandomUtil.isRandomAlsoGV(txtPolarMag.getText()) || NumericUtil.isOneDimNumeric(txtPolarMag.getText());
				if(parse) {
					ModelUtil.runModelChange(new Runnable() {
						public void run() {
							Object bo = Graphiti.getLinkService().getBusinessObjectForLinkedPictogramElement(pe);
		...
		txtPolarArg.addFocusListener(new FocusAdapter() {
			@Override public void focusLost(FocusEvent event) {
				...
				boolean parse = RandomUtil.isRandomAlsoGV(txtPolarArg.getText()) || NumericUtil.isOneDimNumeric(txtPolarArg.getText());
				if(parse) {
					ModelUtil.runModelChange(new Runnable() {
						public void run() {
							Object bo = Graphiti.getLinkService().getBusinessObjectForLinkedPictogramElement(pe);
