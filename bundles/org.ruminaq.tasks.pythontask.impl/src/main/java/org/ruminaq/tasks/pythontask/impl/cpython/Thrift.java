...
public enum Thrift {
    ...
    public String getThrifpy(IProject p, String bin) {
        File thriftpyDir =  new File(p.getLocation().toString() + "/target/thriftpy");
        if(!thriftpyDir.exists()) {
            thriftpyDir.getParentFile().mkdirs();
            try {
                Git g = Git.cloneRepository().setURI(URL)
                                             .setDirectory(thriftpyDir)
                                             .call();
                g.checkout().setName(THRIFTPY_LIB_VERSION).call();

                ProcessBuilder pb = new ProcessBuilder(bin, "setup.py", "develop", "--install-dir=" + thriftpyDir.getParent());
