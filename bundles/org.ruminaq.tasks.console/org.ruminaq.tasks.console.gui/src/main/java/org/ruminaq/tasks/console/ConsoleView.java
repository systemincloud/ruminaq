...
public class ConsoleView implements IView, LaunchListener {
  ...
  public void launched() {
    try {
      ConsoleIService cs = DirmiServer.INSTANCE.getRemote(Util.getUniqueId(console), ConsoleIService.class);
      if(cs != null) cs.addListener(viewApi);
    } catch (RemoteException e) { }
  }

  private ConsoleViewService viewApi = new ConsoleViewService() {
    @Override public void newOutput(final String out) {
    ...
    @Override public void clearScreen() throws RemoteException {
      Display.getDefault().syncExec(new Runnable() {
          public void run() { clear(); }
      });
    }
    @Override public void deleteFirstLine() throws RemoteException {
      Display.getDefault().syncExec(new Runnable() {
          public void run() {
          text.setText(text.getText().substring(text.getLine(0).length() + 1));
          mementoHistory.get(console).delete(0, mementoHistory.get(console).indexOf("\n") + 1);
          text.setSelection(text.getText().length());
          }
      });
    }
  ...
  private void initActions(final Shell shell) {
    text.addVerifyKeyListener(new VerifyKeyListener() {
      @Override public void verifyKey(VerifyEvent e) {
        if(e.keyCode == SWT.ARROW_DOWN)  return;
        if(e.keyCode == SWT.ARROW_LEFT)  return;
        if(e.keyCode == SWT.ARROW_RIGHT) return;
        if(e.keyCode == SWT.ARROW_UP)  return;
    ...
    text.addTraverseListener(new TraverseListener() {
      @Override public void keyTraversed(TraverseEvent event) {
            if(event.detail == SWT.TRAVERSE_RETURN) {
              ...
              ConsoleIService api = DirmiServer.INSTANCE.getRemote(Util.getUniqueId(console), ConsoleIService.class);
          if(api != null)  try { api.newCommand(cmd); } catch (RemoteException e) { }
            }
      }
    });
    btnClear.addSelectionListener(new SelectionAdapter() {
      @Override public void widgetSelected(SelectionEvent e) {
        clear();
        ConsoleIService api = DirmiServer.INSTANCE.getRemote(Util.getUniqueId(console), ConsoleIService.class);
        if(api != null) try { api.clearHistory(); } catch (RemoteException e1) { }
      }
    });
  ...
  public void init(EObject bo, TransactionalEditingDomain ed) {
    ...
    if(history == null) {
      ...
      ConsoleIService api = DirmiServer.INSTANCE.getRemote(Util.getUniqueId(console), ConsoleIService.class);
      if(api != null)  try { history.append(api.getHistory()); } catch (RemoteException e) { }
    ...
    if(textColor == null) { textColor = SWTResourceManager.getColor(SWT.COLOR_GREEN); mementoTextColor.put(console, textColor); }
        text.setForeground(textColor);

    Color bgColor = mementoBackgroundColor.get(bo);
    if(bgColor == null) { bgColor = SWTResourceManager.getColor(SWT.COLOR_BLACK); mementoBackgroundColor.put(console, bgColor); }
  ...
  public void dispose() {
    ...
    public void updateType(){
        /* Prompt */
        if(console.getConsoleType().equals(ConsoleType.IN)) ENTRY = "";
        else                                                ENTRY = ">";
    }
}
