...
public class PropertySection implements IPropertySection {
    ...
    private void initComponents() {
        ...
        cmbType.setItems (new String [] { ConsoleType.IN   .toString(),
                                          ConsoleType.OUT  .toString(),
                                          ConsoleType.INOUT.toString()});

        lblNewLine.setText("New line:");

        newLineCombo.setItems (new String [] { Boolean.TRUE .toString(),
    ...
    private void addStyles() {
        ...
        lblNewLine       .setBackground(SWTResourceManager.getColor(SWT.COLOR_WHITE));
        lblMatricesPretty.setBackground(SWTResourceManager.getColor(SWT.COLOR_WHITE));
        lblNbLines       .setBackground(SWTResourceManager.getColor(SWT.COLOR_WHITE));
        spnNbLines       .setBackground(SWTResourceManager.getColor(SWT.COLOR_WHITE));
    }
    ...
    public void refresh(PictogramElement pe, TransactionalEditingDomain ed) {
        if (pe != null) {
            Object bo = Graphiti.getLinkService().getBusinessObjectForLinkedPictogramElement(pe);
            if (bo == null || !(bo instanceof Console)) return;
            Console console = (Console) bo;

            int i = 0;
            for(String dt : cmbType.getItems()) { if(dt.equals(console.getConsoleType().toString())) break; i++; }
            cmbType.select(i);

            i = 0;
            for(String dt : newLineCombo.getItems()) { if(dt.equals(Boolean.toString(console.isNewLine()))) break; i++; }
            newLineCombo.select(i);

            btnMatricesPretty.setSelection(console.isMatricesPretty());

            spnNbLines.setSelection(console.getNbOfLines());
            ((GridData) spnNbLines.getLayoutData()).widthHint = 20 + 40*String.valueOf(console.getNbOfLines()).length();
            spnNbLines.redraw();
        }
    }
