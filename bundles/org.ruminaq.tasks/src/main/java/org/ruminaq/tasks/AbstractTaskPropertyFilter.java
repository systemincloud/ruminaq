...
public abstract class AbstractTaskPropertyFilter extends AbstractPropertySectionFilter {
	...
	protected boolean accept(PictogramElement pictogramElement) {
		if(Graphiti.getPeService().getPropertyValue(pictogramElement, Constants.LABEL_PROPERTY) != null) return false;
        ...
        if(bo instanceof Task) {
	        ...
	        for(ITaskUiApi t : tasks.getTasks(getPrefix())) {
	        	Version v = Version.parseVersion(task.getVersion().replace(Constants.SNAPSHOT, Constants.QUALIFIER));
	        	...
	        	&& t.checkPropertyFilter(task)) return true;
	        }
        }
        return false;
	}
