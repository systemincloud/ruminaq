...
public class ResizeShapeTaskFeature extends DefaultResizeShapeFeature {
	...
	public void resizeShape(IResizeShapeContext context) {
		...
		for(EObject o : shape.getLink().getBusinessObjects())
			if(AddElementFeature.isLabel(o)) textContainerShape = (ContainerShape) o;
		
		boolean labelInDefaultPosition = AddElementFeature.isLabelInDefaultPosition(textContainerShape, shape); 
		...
		if (shape.getGraphicsAlgorithm() != null) {
			for(GraphicsAlgorithm ga : shape.getGraphicsAlgorithm().getGraphicsAlgorithmChildren()) {
				if(Graphiti.getPeService().getProperty(ga, AddTaskFeature.ICON_DESC_PROPERTY) != null) 
					 Graphiti.getGaService().setLocationAndSize(ga,  0, AddTaskFeature.ICON_SIZE, width, height - AddTaskFeature.ICON_SIZE);
				else Graphiti.getGaService().setLocationAndSize(ga, 0, 0, width, height);
			}
		}
		
		alignInternalPorts(shape, w_before, h_before, width, height);
		
		if(labelInDefaultPosition || isConflictingWithNewSize(textContainerShape, shape)) 
			GraphicsUtil.alignWithShape((MultiText)textContainerShape.getGraphicsAlgorithm().getGraphicsAlgorithmChildren().get(0), 
					                    textContainerShape, 
	...
	private boolean isConflictingWithNewSize(ContainerShape textContainerShape,	Shape shape) {
		return GraphicsUtil.intersectsLabel(textContainerShape, shape);
	}

	private void alignInternalPorts(Shape shape, int w_before, int h_before, int w, int h) {
		
		float w_ratio = (float) w  / (float) w_before;
		float h_ratio = (float) h / (float) h_before;
		
		for(Shape child : ((ContainerShape) shape).getChildren()) {
			String isInternalPort = Graphiti.getPeService().getPropertyValue(child,	Constants.INTERNAL_PORT);
			if (Boolean.parseBoolean(isInternalPort)) {
				int dx = 0;
				int dy = 0;
				int xPort = child.getGraphicsAlgorithm().getX();
				int yPort = child.getGraphicsAlgorithm().getY();
				if(    xPort == 0 || xPort == (w_before - child.getGraphicsAlgorithm().getWidth()) 
					|| yPort == 0 || yPort == (h_before - child.getGraphicsAlgorithm().getHeight())) {
					dy = Math.round(yPort * h_ratio) - yPort;
					if(yPort + dy + child.getGraphicsAlgorithm().getHeight() > h)              dy = h - yPort - child.getGraphicsAlgorithm().getHeight();
					if(yPort + dy + child.getGraphicsAlgorithm().getHeight() < h - MoveInternalPortFeature.EPSILON 
							&& yPort == (h_before - child.getGraphicsAlgorithm().getHeight())) dy = h - yPort - child.getGraphicsAlgorithm().getHeight();
					dx = Math.round(xPort * w_ratio) - xPort;
					if(xPort + dx + child.getGraphicsAlgorithm().getWidth() > w )             dx = w - xPort - child.getGraphicsAlgorithm().getWidth();
					if(xPort + dx + child.getGraphicsAlgorithm().getWidth() < w - MoveInternalPortFeature.EPSILON 
							&& xPort == (w_before - child.getGraphicsAlgorithm().getWidth())) dx = w - xPort - child.getGraphicsAlgorithm().getWidth();
				}
				...
				MoveInternalPortFeature moveFeature = new MoveInternalPortFeature(getFeatureProvider());
