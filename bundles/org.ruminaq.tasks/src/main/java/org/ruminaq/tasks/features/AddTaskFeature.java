...
public abstract class AddTaskFeature extends AddElementFeature {
	...
	public enum InternalPortLabelPosition { LEFT, RIGHT, TOP, BOTTOM; }

	protected int     getHeight()                  { return 70; }
	protected int     getWidth()                   { return 120; }
	protected Style   getStyle()                   { return TaskStyle.getStyle(getDiagram()); }
	protected boolean useIconInsideShape()         { return false; }
	protected String  getInsideIconId()            { return ""; }
	protected String  getInsideIconDesc()          { return null; }
	...
	protected void insertInside(IPeCreateService peCreateService, IGaService gaService, int width, int height, ContainerShape containerShape, GraphicsAlgorithm ga, Task addedTask) {
		if(useIconInsideShape()) insertIconInside(gaService, width, height, ga, addedTask);
		else                     insertNameInside(gaService, width, height, ga, addedTask);
	}

	public AddTaskFeature(IFeatureProvider fp) { super(fp);	}
	...
	private void insertIconInside(IGaService gaService, int width, int height, GraphicsAlgorithm ga, Task addedTask) {
		Image image = gaService.createImage(ga, getInsideIconId());
        gaService.setLocationAndSize(image, (width - ICON_SIZE) / 2, (height - ICON_SIZE) / 2, ICON_SIZE, ICON_SIZE);
        String desc = getInsideIconDesc();
        if(desc != null) {
        	Text descT = gaService.createDefaultText(getDiagram(), ga, desc);
    		Graphiti.getPeService().setPropertyValue(descT, ICON_DESC_PROPERTY, "true");
    		gaService.setLocationAndSize(descT, 0, ICON_SIZE, width, height - ICON_SIZE);
	...
	private void insertNameInside(IGaService gaService, int width, int height, GraphicsAlgorithm ga, Task addedTask) {
		MultiText text = gaService.createDefaultMultiText(getDiagram(), ga, ModelUtil.getName(addedTask.getClass()).replace(" ", "\n"));
	...
	private void addInternalPorts(Task addedTask, ContainerShape parent) {
		...
		if(addedTask.getInputPort().size() > 0) {
			np:for(InternalInputPort iip : addedTask.getInputPort()) {
				for(Field f : pd.getFields()) {
					...
					if(in != null) {
						if( in.n() == 1 && iip.getId().equals(in.name()) ||
						  ( in.n() > 1) && TasksUtil.isMultiplePortId(iip.getId(), in.name()))  {
							switch(in.pos()) {
								case TOP:    topIns   .add(new Pair<InternalInputPort, IN>(iip, in)); break;
								case LEFT:   leftIns  .add(new Pair<InternalInputPort, IN>(iip, in)); break;
								case RIGHT:  rightIns .add(new Pair<InternalInputPort, IN>(iip, in)); break;
								case BOTTOM: bottomIns.add(new Pair<InternalInputPort, IN>(iip, in)); break;
							}
							continue np;
						}
					}
		...
		if(addedTask.getOutputPort().size() > 0) {
			np:for(InternalOutputPort iop : addedTask.getOutputPort()) {
				for(Field f : pd.getFields()) {
					...
					if(out != null) {
						if( out.n() == 1 && iop.getId().equals(out.name()) ||
						  ( out.n() > 1) && TasksUtil.isMultiplePortId(iop.getId(), out.name())) {
							switch(out.pos()) {
								case TOP:    topOuts   .add(new Pair<InternalOutputPort, OUT>(iop, out)); break;
								case LEFT:   leftOuts  .add(new Pair<InternalOutputPort, OUT>(iop, out)); break;
								case RIGHT:  rightOuts .add(new Pair<InternalOutputPort, OUT>(iop, out)); break;
								case BOTTOM: bottomOuts.add(new Pair<InternalOutputPort, OUT>(iop, out)); break;
							}
							continue np;
						}
					}
		...
		if(nbTop > 0) {
			...
			for(Pair<InternalInputPort, IN> ti : topIns) {
				...
				if(ti.getValue0().isAsynchronous()) lineStyle = LineStyle.DOT;
				ContainerShape containerShape = createPictogramForInternalPort(parent, x, y, width, height, getDiagram(), lineWidth, lineStyle);
		        peCreateService.createChopboxAnchor(containerShape);



				ContainerShape portLabelShape = addInternalPortLabel(getDiagram(), parent, ti.getValue0().getId(), width, height, x, y, InternalPortLabelPosition.BOTTOM);
			...
			for(Pair<InternalOutputPort, OUT> to : topOuts) {
				...
				ContainerShape portLabelShape = addInternalPortLabel(getDiagram(), parent, to.getValue0().getId(), width, height, x, y, InternalPortLabelPosition.BOTTOM);
		...
		if(nbLeft > 0) {
			...
			for(Pair<InternalInputPort, IN> li : leftIns) {
				...
				int lineWidth = INPUT_PORT_WIDTH;
				LineStyle lineStyle = LineStyle.SOLID;
				if(li.getValue0().isAsynchronous()) lineStyle = LineStyle.DOT;
				ContainerShape containerShape = createPictogramForInternalPort(parent, x, y, width, height, getDiagram(), lineWidth, lineStyle);
		        peCreateService.createChopboxAnchor(containerShape);

				ContainerShape portLabelShape = addInternalPortLabel(getDiagram(), parent, li.getValue0().getId(), width, height, x, y, InternalPortLabelPosition.RIGHT);
			...
			for(Pair<InternalOutputPort, OUT> lo : leftOuts) {
				...
				ContainerShape portLabelShape = addInternalPortLabel(getDiagram(), parent, lo.getValue0().getId(), width, height, x, y, InternalPortLabelPosition.RIGHT);
		...
		if(nbRight > 0) {
			...
			for(Pair<InternalInputPort, IN> ri : rightIns) {
				...
				int lineWidth       = INPUT_PORT_WIDTH;
				LineStyle lineStyle = LineStyle.SOLID;
				if(ri.getValue0().isAsynchronous()) lineStyle = LineStyle.DOT;
				ContainerShape containerShape = createPictogramForInternalPort(parent, x, y, width, height, getDiagram(), lineWidth, lineStyle);
		        peCreateService.createChopboxAnchor(containerShape);

				ContainerShape portLabelShape = addInternalPortLabel(getDiagram(), parent, ri.getValue0().getId(), width, height, x, y, InternalPortLabelPosition.LEFT);
			...
			for(Pair<InternalOutputPort, OUT> ro : rightOuts) {
				...
				ContainerShape portLabelShape = addInternalPortLabel(getDiagram(), parent, ro.getValue0().getId(), width, height, x, y, InternalPortLabelPosition.LEFT);
		...
		if(nbBottom > 0) {
			...
			for(Pair<InternalInputPort, IN> bi : bottomIns) {
				...
				int lineWidth = INPUT_PORT_WIDTH;
				LineStyle lineStyle = LineStyle.SOLID;
				if(bi.getValue0().isAsynchronous()) lineStyle = LineStyle.DOT;
				ContainerShape containerShape = createPictogramForInternalPort(parent, x, y, width, height, getDiagram(), lineWidth, lineStyle);
		        peCreateService.createChopboxAnchor(containerShape);

				ContainerShape portLabelShape = addInternalPortLabel(getDiagram(), parent, bi.getValue0().getId(), width, height, x, y, InternalPortLabelPosition.TOP);
			...
			for(Pair<InternalOutputPort, OUT> bo : bottomOuts) {
				...
				ContainerShape portLabelShape = addInternalPortLabel(getDiagram(), parent, bo.getValue0().getId(), width, height, x, y, InternalPortLabelPosition.TOP);
	...
	public static ContainerShape createPictogramForInternalPort(ContainerShape parent, int x, int y, int width, int height, Diagram diagram, int lineWidth, LineStyle lineStyle) {
		final IPeCreateService peCreateService = Graphiti.getPeCreateService();
		final IGaService gaService = Graphiti.getGaService();

		final ContainerShape containerShape = peCreateService.createContainerShape(parent, true);
		final RoundedRectangle invisibleRectangle = gaService.createRoundedRectangle(containerShape, 5, 5);
		...
		final RoundedRectangle rectangle = gaService.createRoundedRectangle(invisibleRectangle, 5, 5);
		...
		Graphiti.getPeService().setPropertyValue(containerShape, Constants.INTERNAL_PORT, "true");
	...
	public static ContainerShape addInternalPortLabel(Diagram diagram,
		...
		ContainerShape textContainerShape = peService.createContainerShape(parent, true);
		...
		switch (position) {
			case RIGHT  : GraphicsUtil.onRightOfShape (text, textContainerShape, width, height, x, y, 0, 0); break;
			case LEFT   : GraphicsUtil.onLeftOfShape  (text, textContainerShape, width, height, x, y, 0, 0); break;
			case TOP    : GraphicsUtil.onTopOfShape   (text, textContainerShape, width, height, x, y, 0, 0); break;
			case BOTTOM : GraphicsUtil.onBottomOfShape(text, textContainerShape, width, height, x, y, 0, 0); break;
		}

		Graphiti.getPeService().setPropertyValue(textContainerShape, Constants.PORT_LABEL_PROPERTY, "true");

		return textContainerShape;
	}

	public static void distributePortsOnLeft(ContainerShape parent, IFeatureProvider fp) {
		distributePortsVertically(parent, 0, fp);
	}

	public static void distributePortsOnRight(ContainerShape parent, IFeatureProvider fp) {
		distributePortsVertically(parent, parent.getGraphicsAlgorithm().getWidth() - PORT_SIZE, fp);
	}

	public static void distributePortsOnTop(ContainerShape parent, IFeatureProvider fp) {
		distributePortsHorizontally(parent, 0, fp);
	}

	public static void distributePortsOnBottom(ContainerShape parent, IFeatureProvider fp) {
		distributePortsHorizontally(parent, parent.getGraphicsAlgorithm().getHeight() - PORT_SIZE, fp);
	}

	private static void distributePortsVertically(ContainerShape parent , int x, IFeatureProvider fp) {
			...
			if(isInternalPortLabel(child)) {
				if(child.getGraphicsAlgorithm().getX() == x) {
					for(Shape s : orderedChilds) {
						if(child.getGraphicsAlgorithm().getY() < s.getGraphicsAlgorithm().getY()) {
		...
		if(orderedChilds.size() != 0) {
			int stepPorts = parent.getGraphicsAlgorithm().getHeight() / orderedChilds.size();
	...
	private static void distributePortsHorizontally(ContainerShape parent , int y, IFeatureProvider fp) {
		LinkedList<Shape> orderedChilds = new LinkedList<>();
		...
		loop:for(Shape child : parent.getChildren()) {
			if(isInternalPortLabel(child)) {
				if(child.getGraphicsAlgorithm().getY() == y) {
					for(Shape s : orderedChilds) {
						if(child.getGraphicsAlgorithm().getX() < s.getGraphicsAlgorithm().getX()) {
							orderedChilds.add(orderedChilds.indexOf(s), child);
							continue loop;
						}
					}
		...
		if(orderedChilds.size() != 0) {
			int stepPorts = parent.getGraphicsAlgorithm().getWidth() / orderedChilds.size();
			int position = (stepPorts >> 1) - (PORT_SIZE >> 1);
			for(Shape child : orderedChilds) {
	...
	public static PictogramElement getPictogramElementOfInternalPort(Diagram diagram, InternalPort ip) {
		PictogramElement ret = null;
		for(PictogramElement pe : Graphiti.getLinkService().getPictogramElements(diagram, ip))
			if(isInternalPortLabel(pe)) { ret = pe; break; }
		return ret;
	}

	public static Shape getPictogramElementOfInternalPort(ContainerShape parent, InternalPort internalPort) {
		for(Shape s : parent.getChildren())
			if(isInternalPortLabel(s) && s.getLink().getBusinessObjects().get(0) == internalPort) return s;
		return null;
	}

	public static Position getPosition(ContainerShape parent, InternalPort internalPort) {
		for(Shape s : parent.getChildren()) {
			if(isInternalPortLabel(s) && s.getLink().getBusinessObjects().get(0) == internalPort) {
				...
				if(x == W - w) return Position.RIGHT;
				if(y == 0)     return Position.TOP;
				if(y == H - h) return Position.BOTTOM;
			}
		}
		return null;
	}
	...
	public static boolean isInternalPortLabel(PictogramElement pe) {
		return Graphiti.getPeService().getPropertyValue(pe, Constants.INTERNAL_PORT) != null;
	}
}
