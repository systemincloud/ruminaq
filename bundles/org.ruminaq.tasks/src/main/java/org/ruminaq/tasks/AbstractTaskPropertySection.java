...
public abstract class AbstractTaskPropertySection extends GFPropertySection implements ITabbedPropertyConstants {
	...
	protected ITaskUiApi getTaskUiApi() {
        ...
        if(bo instanceof Task) {
	        ...
	        for(ITaskUiApi t : tasks.getTasks(getPrefix())) {
	        	...
	        	&& TaskProvider.compare(t.getVersion(), Version.parseVersion(task.getVersion()))) return t;
	        }
        }
        return null;
	}
	...
	public void refresh() {
		...
		super.refresh();
		if(propertySection != null) propertySection.refresh(getSelectedPictogramElement(),
				                                            getDiagramContainer().getDiagramBehavior().getEditingDomain());
	}
    ...
    public void aLaunched()     { Display.getDefault().asyncExec(new Runnable() { public void run() {
    	notChanged = new LinkedList<>();
    	if(!parent.isDisposed()) EclipseUtil.setEnabledRecursive(parent, false, notChanged); }});
    }

	public void aStopped()      { Display.getDefault().asyncExec(new Runnable() { public void run() {
		if(!parent.isDisposed()) {
			EclipseUtil.setEnabledRecursive(parent, true, new LinkedList<Control>());
			for(Control c : notChanged) c.setEnabled(false);
		}}});
	}

	public void aDirmiStarted() { }
}
