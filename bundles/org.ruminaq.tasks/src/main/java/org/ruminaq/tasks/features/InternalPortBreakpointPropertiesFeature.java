...
public class InternalPortBreakpointPropertiesFeature extends AbstractCustomFeature {
	...
	public static void doExecute(ICustomContext context, IFeatureProvider fp) {
		...
		Object bo = fp.getBusinessObjectForPictogramElement(((ICustomContext) context).getPictogramElements()[0]);
			...
			if(bo != null && bo instanceof InternalPort) {
				...
				for(int i = 0; i < breakpoints.length; i++) {
					...
					if(resource.equals(breakpoint.getMarker().getResource())) {
						if(breakpoint.getMarker().getAttribute(InternalPortBreakpoint.TASK_ID).equals(ip.getParent().getId()) &&
						   breakpoint.getMarker().getAttribute(InternalPortBreakpoint.PORT_ID).equals(ip.getId())) {
							PropertyDialogAction action = new PropertyDialogAction(new SameShellProvider(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell()), new ISelectionProvider() {
								public void       addSelectionChangedListener(ISelectionChangedListener listener)    { }
								public ISelection getSelection()                                                     { return new StructuredSelection(breakpoint); }
