...
public class PropertyInternalInputPortSection extends GFPropertySection implements ITabbedPropertyConstants {
    ...
    private void initLayout(Composite parent) {
        ...
        GridData lytQuequeSize = new GridData(SWT.LEFT, SWT.CENTER, false, false, 1, 1);
        lytQuequeSize.minimumWidth = 25; lytQuequeSize.widthHint = 25;
    ...
    private void initActions() {
        ...
        btnIgnoreLossyCast.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                ...
                }, getDiagramContainer().getDiagramBehavior().getEditingDomain(), "Change console type");
            }});
        txtQueueSize.addFocusListener(new FocusAdapter() {
            @Override public void focusLost(FocusEvent event) {
                ...
                if (bo == null || !(bo instanceof InternalInputPort)) return;
                final InternalInputPort iip = (InternalInputPort) bo;
                if(parse) {
                    ModelUtil.runModelChange(new Runnable() {
                        public void run() {
                            iip.setQueueSize(txtQueueSize.getText());
                            btnDefaultQueueSize.setEnabled(!iip.getDefaultQueueSize().equals(iip.getQueueSize()));
                        }
        ...
        txtQueueSize.addTraverseListener(new TraverseListener() {
            @Override public void keyTraversed(TraverseEvent event) { if(event.detail == SWT.TRAVERSE_RETURN) btnIgnoreLossyCast.setFocus(); }
        });
                ...
                ModelUtil.runModelChange(new Runnable() {
                    public void run() {
                        ...
                        if (bo instanceof InternalInputPort) {
                            ...
                            btnDefaultHoldLast.setEnabled(iip.isDefaultHoldLast() != iip.isHoldLast());
                        }
                    }
                }, getDiagramContainer().getDiagramBehavior().getEditingDomain(), "Change console type");
            }});
    ...
    private void addStyles() {
        ...
        lblAsynchronousValue.setFont(JFaceResources.getFontRegistry().getItalic(""));
    }
    ...
    public void refresh() {
        ...
        if (pe != null) {
            ...
            if(dataType.length() > 2) dataType.delete(dataType.length() - 2, dataType.length());

            dataTypeValue.setText(dataType.toString());

            lblAsynchronousValue.setText(Boolean.toString(ip.isAsynchronous()));
            lblGroupValue.setText(ip.getGroup() == -1 ? "None" : Integer.toString(ip.getGroup()));
            ...
            boolean quequeVisible = !((ip.isAsynchronous() || !ip.getParent().isAtomic()) && ip.getParent() instanceof EmbeddedTask);
            lblQueueSize      .setVisible(quequeVisible);
            cmpQueueSize      .setVisible(quequeVisible);
            if(quequeVisible)
                btnDefaultQueueSize.setEnabled(!ip.getDefaultQueueSize().equals(ip.getQueueSize()));

            boolean holdVisible = !(ip.isAsynchronous() || !ip.getParent().isAtomic());
            lblHoldLast       .setVisible(holdVisible);
            cmpHoldLast       .setVisible(holdVisible);
            if(holdVisible)
                btnDefaultHoldLast.setEnabled(ip.isDefaultHoldLast() != ip.isHoldLast());
