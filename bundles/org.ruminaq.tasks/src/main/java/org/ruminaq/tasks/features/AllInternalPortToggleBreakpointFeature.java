...
public class AllInternalPortToggleBreakpointFeature extends AbstractCustomFeature {
					...
					for(InternalInputPort p : t.getInputPort()) {
						...
						for(int i = 0; i < breakpoints.length; i++) {
							...
							if(resource.equals(breakpoint.getMarker().getResource())) {
								if(breakpoint.getMarker().getAttribute(Task.class.getSimpleName()).equals(t.getId()) &&
								   breakpoint.getMarker().getAttribute(InternalPort.class.getSimpleName()).equals(p.getId())) {
									breakpoint.delete();
									tmp = true;
								}
							}
						...
						if(!tmp) {
							InternalPortBreakpoint internalPortBreakpoint = new InternalPortBreakpoint(resource, path, t.getId(), p.getId());
