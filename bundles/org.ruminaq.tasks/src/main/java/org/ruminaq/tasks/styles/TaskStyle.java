...
public class TaskStyle {
	...
	private static final IColorConstant TASK_FOREGROUND = new ColorConstant(0, 0, 0);
	private static final IColorConstant TASK_BACKROUND  = new ColorConstant(255, 255, 255);
	private static final IColorConstant ONLY_LOCAL_TASK_BACKROUND = new ColorConstant(220, 220, 220);
	...
	final String styleId = "ONLY_LOCAL_TASK"; //$NON-NLS-1$
		...
		if(style == null && create) {
			...
			style = gaService.createStyle(diagram, styleId);
			style.setForeground(gaService.manageColor(diagram, TASK_FOREGROUND));
			style.setBackground(gaService.manageColor(diagram, ONLY_LOCAL_TASK_BACKROUND));
