...
public abstract class CreateTaskFeature extends CreateElementFeature {
    ...
    private final Logger logger = ModelerLoggerFactory.getLogger(CreateTaskFeature.class);

    private String  bundleName;
    private Version version;

	public CreateTaskFeature(IFeatureProvider fp, String name, String description, String bundleName, Version version) {
		super(fp, name, description);
		this.bundleName = bundleName;
		this.version    = version;
	}

	public CreateTaskFeature(IFeatureProvider fp, Class<? extends Task> clazz, String bundleName, Version version) {
		super(fp, clazz);
		this.bundleName = bundleName;
		this.version    = version;
	}
	...
	protected Object[] create(ICreateContext context, Task task) {
	    ...
	    task.setVersion(version.getMajor() + "." + version.getMinor() + "." + version.getMicro());
		...
		for(Field f : pd.getFields()) {
			...
			if(in != null) {
				...
				for(int i = 0; i < in.n(); i++) {
					InternalInputPort inputPort = (InternalInputPort) RuminaqFactory.eINSTANCE.createInternalInputPort();
					String id = in.name();
					...
					int group = in.group();
					if(in.n() > 1) {
						if(in.ngroup().equals(NGroup.SAME)) inputPort.setGroup(group);
						else {
							if(group == -1) inputPort.setGroup(group);
							else {
								int j = group;
								boolean free = true;
								do {
					...
					for(Class<? extends DataType> dt : in.type())
						try {
							EFactory factory = (EFactory) in.factory().getDeclaredField("eINSTANCE").get(null);
							Method createMethod = factory.getClass().getMethod("create" + dt.getSimpleName(), (Class<?>[]) null);
							inputPort.getDataType().add((DataType) createMethod.invoke(factory,(Object[]) null));
			...
			if(out != null) {
				...
				for(int i = 0; i < out.n(); i++) {
					InternalOutputPort outputPort = (InternalOutputPort) RuminaqFactory.eINSTANCE.createInternalOutputPort();
					String id = out.name();
					...
					for(Class<? extends DataType> dt : out.type())
						try {
							EFactory factory = (EFactory) out.factory().getDeclaredField("eINSTANCE").get(null);
							Method createMethod = factory.getClass().getMethod("create" + dt.getSimpleName(), (Class<?>[]) null);
							outputPort.getDataType().add((DataType) createMethod.invoke(factory,(Object[]) null));
						} catch (SecurityException | NoSuchMethodException | IllegalAccessException | IllegalArgumentException | InvocationTargetException | NoSuchFieldException e) { }
