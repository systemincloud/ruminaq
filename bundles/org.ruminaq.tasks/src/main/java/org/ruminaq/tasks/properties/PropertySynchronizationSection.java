...
public class PropertySynchronizationSection extends GFPropertySection implements ITabbedPropertyConstants {
	...
	private static Image getImage(String file) {
		Bundle bundle = FrameworkUtil.getBundle(PropertySynchronizationSection.class);
		URL url = FileLocator.find(bundle, new Path("icons/" + file), null);
		ImageDescriptor image = ImageDescriptor.createFromURL(url);
		return image.createImage();
	}

    private final class SynchronizationContentProvider implements ITreeContentProvider {

		@Override
		public void dispose() {
			for(Button b : buttons.values()) b.dispose();
		}
		...
		public Object[] getChildren(Object o) {
			return ((InternalOutputPort) o).getSynchronization().toArray(new Synchronization[((InternalOutputPort) o).getSynchronization().size()]);
		}

		@Override
		@SuppressWarnings("unchecked")
		public Object[] getElements(Object o) {
			return ((EObjectContainmentEList<InternalOutputPort>) o).toArray(new InternalOutputPort[((EObjectContainmentEList<InternalOutputPort>) o).size()]);
		}
		...
		public boolean hasChildren(Object o) {
			if(o instanceof InternalOutputPort) return ((InternalOutputPort) o).getSynchronization().size() > 0;
			else                                return false;
		}

	}

	private final class TreeLabelProvider extends LabelProvider implements ITableLabelProvider {
		@Override public String getColumnText(Object o, int columnIndex) {
			if(o instanceof InternalOutputPort) {
				InternalOutputPort op = (InternalOutputPort) o;
				switch(columnIndex) {
					case 0:  return op.getId();
					case 10: return op.getResetPort() == null ? NONE : op.getResetPort().getParent().getId();
					case 11: return op.getResetPort() == null ? NONE : op.getResetPort() instanceof InternalOutputPort ? OUT + op.getResetPort().getId() :
				...
				switch(columnIndex) {
					case 1:  return s.getGroup() == 0 ? NONE : "" + s.getGroup();
					case 2:  return s.getWaitForPort() == null ? NONE : s.getWaitForPort().getParent().getId();
					case 3:  return s.getWaitForPort() == null ? NONE : s.getWaitForPort() instanceof InternalOutputPort ? OUT + s.getWaitForPort().getId() :
					                                    	                                                               IN  + s.getWaitForPort().getId();
					case 4:  return s.getWaitForTicks();
					case 6:  return s.getSkipFirst();
					default: return "";
				}
			}
			return "";
		}
		@Override public Image getColumnImage(Object paramObject, int paramInt) { return null; }
        @Override
	...
	private final class GroupEditingSupport extends EditingSupport {
	    ...
	    @Override protected CellEditor getCellEditor(Object o) { return cellEditor; }
	    @Override protected boolean    canEdit      (Object o) { return o instanceof Synchronization; }
	    @Override protected Object     getValue     (Object o) {
	        if(o instanceof Synchronization) return "" + ((Synchronization) o).getGroup();
	        else                             return "";
	    }
	    @Override protected void setValue(Object o, final Object value) {
	        if (o instanceof Synchronization) {
	        	if(!NumericUtil.isOneDimPositiveInteger((String) value)) return;
				...
				}, getDiagramContainer().getDiagramBehavior().getEditingDomain(), "Change");
	        }
	    }
	}
	...
	private class TaskEditingSupport extends EditingSupport {
	    ...
	    private MainTask mt;
	    private TaskEditingSupport(ColumnViewer viewer, MainTask mt, final Task thisTask) {
	        super(viewer);
	        this.mt = mt;
	        ...
	        for(int i = 1; i <= mt.getTask().size(); i++) {
	        	if(mt.getTask().get(i - 1) == thisTask) continue;
	    ...
	    @Override protected Object     getValue     (Object o) {
	        if(o instanceof Synchronization) {
	        	Synchronization s = (Synchronization) o;
	            return s.getWaitForPort() != null ? s.getWaitForPort().getParent().getId() : NONE;
	        }
	        return null;
	    }
	    @Override protected void setValue(Object o, final Object value) {
	        if(o instanceof Synchronization) {
				...
				ModelUtil.runModelChange(new Runnable() {
                    ...
                    public void run() {
			            ...
			            if(NONE.equals(newValue)) s.setWaitForPort(null);
			            else {
			            	for(Task t : mt.getTask()) {
								if(t.getId().equals(newValue)) {
									     if(t.getOutputPort().size() > 0) s.setWaitForPort(t.getOutputPort().get(0));
									else if(t.getInputPort() .size() > 0) s.setWaitForPort(t.getInputPort() .get(0));
								}
							}
			            }
			            treVwOutputPorts.refresh();
				        for(TreeColumn tc : treOutputPorts.getColumns()) tc.pack();
				        root.layout();
					}
				}, getDiagramContainer().getDiagramBehavior().getEditingDomain(), "Change");
	        }
	...
	private class PortEditingSupport extends EditingSupport {
	    private PortEditingSupport(ColumnViewer viewer) { super(viewer); }
	    	...
	    	if(o instanceof Synchronization) {
		        ...
		        if(s.getWaitForPort() != null) {
		        	...
		        	if(size > 0) {
		        		String[] inputs = new String[size];
		        		int k = 0;
		        		for(int i = 0; i < t.getInputPort() .size(); i++) inputs[k++] = IN  + t.getInputPort() .get(i).getId();
		        		for(int i = 0; i < t.getOutputPort().size(); i++) inputs[k++] = OUT + t.getOutputPort().get(i).getId();
		        		cellEditor.setInput(inputs);
		        		return cellEditor;
		        	}
		        }
	    ...
	    @Override protected Object  getValue(Object o) {
	        if(o instanceof Synchronization) {
	        	Synchronization s = (Synchronization) o;
	            return s.getWaitForPort() == null ? NONE : s.getWaitForPort() instanceof InternalOutputPort ? OUT + s.getWaitForPort().getId() :
                                                                                                              IN  + s.getWaitForPort().getId();
	        }
	        return null;
	    }
				...
				ModelUtil.runModelChange(new Runnable() {
                    ...
                    public void run() {
						...
						if(newValue.startsWith(IN)) {
							for(InternalInputPort ip : s.getWaitForPort().getParent().getInputPort())
								if(ip.getId().equals(newValue.substring(IN.length()))) s.setWaitForPort(ip);

						} else if(newValue.startsWith(OUT)) {
							for(InternalOutputPort op : s.getWaitForPort().getParent().getOutputPort())
								if(op.getId().equals(newValue.substring(OUT.length()))) s.setWaitForPort(op);
						}
	...
	private class ResetTaskEditingSupport extends EditingSupport {
	    private ComboBoxViewerCellEditor cellEditor = null;
	    private MainTask mt;
	    private ResetTaskEditingSupport(ColumnViewer viewer, MainTask mt, final Task thisTask) {
	        super(viewer);
	        this.mt = mt;
	        cellEditor = new ComboBoxViewerCellEditor((Composite) getViewer().getControl(), SWT.READ_ONLY);
	        cellEditor.setLabelProvider(new LabelProvider());
	    ...
	    @Override protected CellEditor getCellEditor(Object o) { return cellEditor; }
	    @Override protected boolean    canEdit      (Object o) { return o instanceof InternalOutputPort; }
	    @Override protected Object     getValue     (Object o) {
	        if(o instanceof InternalOutputPort) {
	        	InternalOutputPort el = (InternalOutputPort) o;
	            return el.getResetPort() != null ? el.getResetPort().getParent().getId() : NONE;
	        }
	        return null;
	    }
	    @Override protected void setValue(Object o, final Object value) {
	        if(o instanceof InternalOutputPort) {
				...
				ModelUtil.runModelChange(new Runnable() {
                    ...
                    public void run() {
						String newValue = (String) value;
			            if(NONE.equals(newValue)) op.setResetPort(null);
			            else {
			            	for(Task t : mt.getTask()) {
								if(t.getId().equals(newValue)) {
									     if(t.getOutputPort().size() > 0) op.setResetPort(t.getOutputPort().get(0));
									else if(t.getInputPort() .size() > 0) op.setResetPort(t.getInputPort() .get(0));
								}
							}
			            }
			            treVwOutputPorts.refresh();
	...
	private class ResetPortEditingSupport extends EditingSupport {
	    private ResetPortEditingSupport(ColumnViewer viewer) { super(viewer); }
	    @Override protected CellEditor getCellEditor(Object o) {
	    	if(o instanceof InternalOutputPort) {
		    	ComboBoxViewerCellEditor cellEditor = new ComboBoxViewerCellEditor((Composite) getViewer().getControl(), SWT.READ_ONLY);
		    	InternalOutputPort el = (InternalOutputPort) o;
	    ...
	    @Override protected Object  getValue(Object o) {
	        if(o instanceof InternalOutputPort) {
	        	InternalOutputPort el = (InternalOutputPort) o;
	            return el.getResetPort() == null ? NONE : el.getResetPort() instanceof InternalOutputPort ? OUT + el.getResetPort().getId() :
	        ...
	        if(o instanceof InternalOutputPort) {
				...
				ModelUtil.runModelChange(new Runnable() {
                    ...
                    public void run() {
						...
						if(newValue.startsWith(IN))
							for(InternalInputPort ip : el.getResetPort().getParent().getInputPort())
								if(ip.getId().equals(newValue.substring(IN.length()))) el.setResetPort(ip);

						else if(newValue.startsWith(OUT))
							for(InternalOutputPort op : el.getResetPort().getParent().getOutputPort())
								if(op.getId().equals(newValue.substring(OUT.length()))) el.setResetPort(op);
	...
	private class SkipEditingSupport extends EditingSupport {
	    ...
	    @Override protected boolean    canEdit      (Object o) { return o instanceof Synchronization; }
	    @Override protected Object     getValue     (Object o) {
	        if(o instanceof Synchronization) return get((Synchronization) o);
	        else                             return null;
	    }
	    @Override protected void setValue(Object o, final Object value) {
	        if (o instanceof Synchronization) {
	    ...
	    protected Object get(Synchronization s)               { return s.getSkipFirst(); }
	    protected void   set(Synchronization s, String value) {        s.setSkipFirst(value); }
	}
	...
	private class NotifyEditingSupport extends SkipEditingSupport {
	    ...
	    @Override protected Object get(Synchronization s)               { return s.getWaitForTicks(); }
	    @Override protected void   set(Synchronization s, String value) {        s.setWaitForTicks(value); }
	}
	...
	private final class NotifyLoopEditingSupport extends EditingSupport {
	    ...
	    private NotifyLoopEditingSupport(ColumnViewer viewer) {
	        super(viewer);
	        cellEditor = new CheckboxCellEditor((Composite) getViewer().getControl(), SWT.CHECK);
	    }
	    @Override protected CellEditor getCellEditor(Object o) { return cellEditor; }
	    ...
	    @Override protected void setValue(Object o, Object value) {
	        ...
	        if(o instanceof InternalOutputPort) {
				...
				ModelUtil.runModelChange(new Runnable() {
                    ...
                    public void run() {
						...
						if(boolValue)
			            	for(Synchronization s : op.getSynchronization()) s.setLoop(false);
			            treVwOutputPorts.refresh();
					}
				}, getDiagramContainer().getDiagramBehavior().getEditingDomain(), "Change");
	        } else if(o instanceof Synchronization) {
	        	final Synchronization s = (Synchronization) o;
	        	if(s.getParent().getSynchronization().lastIndexOf(o) != s.getParent().getSynchronization().size() - 1) return;
				ModelUtil.runModelChange(new Runnable() {
					@Override
                    public void run() {
			            s.setLoop(boolValue);
	        ...
	        if(o instanceof InternalOutputPort) {
				...
				ModelUtil.runModelChange(new Runnable() {
                    ...
                    public void run() {
			            op.setLoop(boolValue);
						if(boolValue)
			            	for(Synchronization s : op.getSynchronization()) s.setSkipLoop(false);
			            treVwOutputPorts.refresh();
					}
				}, getDiagramContainer().getDiagramBehavior().getEditingDomain(), "Change");
	        } else if(o instanceof Synchronization) {
	...
	private void initLayout(Composite parent) {
		...
		treOutputPorts.setLayoutData(new GridData(SWT.FILL, SWT.FILL, false, true, 1, 1));
	...
	private void initActions() {
		treOutputPorts.addMouseListener(new MouseListener(){
			...
			public void mouseUp(MouseEvent arg0) {
				...
				if(cell != null){
					TreeColumn clickedColumn = treOutputPorts.getColumn(cell.getColumnIndex());
					...
					if(clickedColumn == column){
		                ...
		                if(o instanceof InternalOutputPort) {
		                	...
		                	final Synchronization s = RuminaqFactory.eINSTANCE.createSynchronization();
	            			ModelUtil.runModelChange(new Runnable() {
	        					@Override
                                public void run() {
	        						if(iop.getSynchronization().size() != 0)
	        							iop.getSynchronization().get(iop.getSynchronization().size() - 1).setLoop(false);
	        						if(iop.getSynchronization().size() != 0)
	        							iop.getSynchronization().get(iop.getSynchronization().size() - 1).setSkipLoop(false);

	        						s.setParent(iop);
	        						iop.getSynchronization().add(s);
	        					}
	        				}, getDiagramContainer().getDiagramBehavior().getEditingDomain(), "Add Synchronization");
	        				...
	        				}, getDiagramContainer().getDiagramBehavior().getEditingDomain(), "Remove synchronization");
			                for(TreeItem it1 : treOutputPorts.getItems())
			                	for(TreeItem it2 : it1.getItems())
			                		if(it2.getData() == snc) it2.dispose();
			                if(buttons.remove(snc) != null) buttons.remove(snc).dispose();
			                refresh();
		                }
					}
				}
	...
	private void initComponents() {
		...
		treclVwOutputPortsNLoop.setLabelProvider(new ColumnLabelProvider() {
		      @Override public String getText(Object o)  { return null; }
		      @Override public Image  getImage(Object o) {
		    	  if(o instanceof InternalOutputPort) {
		    		  if(((InternalOutputPort) o).isLoop()) return CHECKED;
		    		  else                                  return UNCHECKED;
		    	  } else if(o instanceof Synchronization) {
		    		  Synchronization s = (Synchronization) o;
		    		  if(s.getParent().getSynchronization().lastIndexOf(o) == s.getParent().getSynchronization().size() - 1) {
		    			  if(s.isLoop()) return CHECKED;
		    			  else           return UNCHECKED;
		    		  } else return null;
		    	  } else return null;
		      }});
		...
		treclVwOutputPortsSLoop.setLabelProvider(new ColumnLabelProvider() {
		      ...
		      @Override public Image  getImage(Object o) {
		    	  if(o instanceof Synchronization) {
		    		  Synchronization s = (Synchronization) o;
		    		  if(s.isSkipLoop()) return CHECKED;
		    		  else               return UNCHECKED;
		    	  } else return null;
		      }});
		...
		treclVwOutputPortsBtn.setLabelProvider(new ColumnLabelProvider() {
			@Override public String getText(Object o)  {
		    	  if(o instanceof InternalOutputPort) return "+";
		    	  else                                return "-";
		      }
        });
	...
	@Override
	public void refresh() {
		if(treOutputPorts.isDisposed()) return;
		...
		if(pe != null) {
			...
			treclEdOutputPortsGrp  = new GroupEditingSupport(treclVwOutputPortsGrp.getViewer());
			treclVwOutputPortsGrp.setEditingSupport(treclEdOutputPortsGrp);

			treclEdOutputPortsPort = new PortEditingSupport(treclVwOutputPortsPort.getViewer());
			treclVwOutputPortsPort.setEditingSupport(treclEdOutputPortsPort);

			treclEdOutputPortsTask = new TaskEditingSupport(treclVwOutputPortsTask.getViewer(), ModelHandler.getModel(getDiagramTypeProvider().getDiagram(),
					                                                                                                  getDiagramTypeProvider().getFeatureProvider()),
					                                                                                                  t);
			treclVwOutputPortsTask.setEditingSupport(treclEdOutputPortsTask);

			treclEdOutputPortsSkip = new SkipEditingSupport(treclVwOutputPortsSkip.getViewer());
			treclVwOutputPortsSkip.setEditingSupport(treclEdOutputPortsSkip);

			treclEdOutputPortsSLoop = new SkipLoopEditingSupport(treclVwOutputPortsSLoop.getViewer());
			treclVwOutputPortsSLoop.setEditingSupport(treclEdOutputPortsSLoop);

			treclEdOutputPortsNotify = new NotifyEditingSupport(treclVwOutputPortsNotify.getViewer());
			treclVwOutputPortsNotify.setEditingSupport(treclEdOutputPortsNotify);

			treclEdOutputPortsNLoop = new NotifyLoopEditingSupport(treclVwOutputPortsNLoop.getViewer());
			treclVwOutputPortsNLoop.setEditingSupport(treclEdOutputPortsNLoop);

			treclEdOutputResetPort = new ResetPortEditingSupport(treclVwOutputResetPort.getViewer());
			treclVwOutputResetPort.setEditingSupport(treclEdOutputResetPort);

			treclEdOutputResetTask = new ResetTaskEditingSupport(treclVwOutputResetTask.getViewer(), ModelHandler.getModel(getDiagramTypeProvider().getDiagram(),
					                                                                                                       getDiagramTypeProvider().getFeatureProvider()),
					                                                                                                       t);
			treclVwOutputResetTask.setEditingSupport(treclEdOutputResetTask);
		}
		for(TreeItem   ti : treVwOutputPorts.getTree().getItems()) ti.setExpanded(true);
		treVwOutputPorts.refresh();
    	for(TreeColumn tc : treOutputPorts.getColumns())           tc.pack();
    	root.layout();
	}
