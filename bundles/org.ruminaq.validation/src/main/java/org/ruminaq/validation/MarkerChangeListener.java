...
public class MarkerChangeListener implements IResourceChangeListener {
	...
	public MarkerChangeListener(IFile file, TransactionalEditingDomain ed, DiagramBehavior db, Display dispaly) {
		this.file = file;
		this.ed = ed;
		this.db = db;
	...
	public void resourceChanged(IResourceChangeEvent event) {
		if(file == null) return;
        final IResourceDelta modelFileDelta = event.getDelta().findMember(file.getFullPath());
        if (modelFileDelta == null) return;
        final IMarkerDelta[] markerDeltas = modelFileDelta.getMarkerDeltas();
        if (markerDeltas == null || markerDeltas.length == 0) return;
        ...
        for (IMarkerDelta markerDelta : markerDeltas) {
            ...
            case IResourceDelta.REMOVED:
                ...
                if (uri != null) deletedMarkers.add(uri);
            }
        }

        final Set<EObject> updatedObjects = new LinkedHashSet<>();
        for (String uri : deletedMarkers) {
        	if(ed == null || ed.getResourceSet() == null) continue;
            final EObject eobject = ed.getResourceSet().getEObject(URI.createURI(uri), false);
            if (eobject == null) continue;
            final ValidationStatusAdapter adapter = (ValidationStatusAdapter) EcoreUtil.getRegisteredAdapter(eobject, ValidationStatusAdapter.class);
