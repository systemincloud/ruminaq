...
public class AddSimpleConnectionFeature extends AbstractAddFeature {
    ...
    public boolean canAdd(IAddContext context) {
        if (context instanceof IAddConnectionContext && context.getNewObject() instanceof SimpleConnection) return true;
        else return false;
    }
    ...
    public PictogramElement add(IAddContext context) {
        ...
        SimpleConnection addedSimpleConnection = (SimpleConnection) context.getNewObject();
	...
	public void linkToConnectionBeforePoint(Connection connection,	SimpleConnection addedSimpleConnection) {
		String connectionPointPropertyStart = Graphiti.getPeService().getPropertyValue(connection.getStart().getParent(), Constants.SIMPLE_CONNECTION_POINT);
		if(Boolean.parseBoolean(connectionPointPropertyStart)) {
			for(Connection c : connection.getStart().getIncomingConnections()) {
		        link(c, Util.concat(c.getLink().getBusinessObjects().toArray(), addedSimpleConnection));
