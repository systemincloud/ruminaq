...
public abstract class AddElementFeature extends AbstractAddShapeFeature {
	...
	protected ContainerShape addLabel(ContainerShape targetContainer,
                                              String label,
                                                 ...
                                                 int shapeX,
                                                 int shapeY) {
		return addLabel(targetContainer, getDiagram(), label, width, height, shapeX, shapeY);
	}
	...
	public static ContainerShape addLabel(ContainerShape targetContainer,
		...
		GraphicsUtil.alignWithShape(text, textContainerShape, width, height, shapeX, shapeY, 0, 0);

		Graphiti.getPeService().setPropertyValue(textContainerShape, Constants.LABEL_PROPERTY, "true");

		return textContainerShape;
	}

	public static boolean isLabel(Object o) {
		return o instanceof ContainerShape && Graphiti.getPeService().getPropertyValue((ContainerShape) o, Constants.LABEL_PROPERTY) != null;
	}

	public static boolean isLabelInDefaultPosition(ContainerShape label, Shape shape) {
