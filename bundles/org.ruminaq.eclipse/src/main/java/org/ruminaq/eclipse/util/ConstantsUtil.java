...
public class ConstantsUtil {
	...
	public static boolean isInTestDirectory(IFile file) {
		...
		folder = sb.toString();
		if(folder.startsWith(SourceFolders.TEST_DIAGRAM_FOLDER)) return true;
		else                                                    return false;
	}
	...
	public static boolean isInTestDirectory(URI uri, String basePath) {
		...
		if(file != null) folder = file.replace(basePath, "");
		if(folder.contains(":")) folder = folder.substring(folder.indexOf(":") + 1);
	...
	public static boolean isTest(URI uri) {
		if(Constants.EXTENSION.equals(uri.fileExtension()) && isInTestDirectory(uri)) return true;
		else return false;
	}

	public static boolean isTest(IFile file) {
		if(Constants.EXTENSION.equals(file.getFileExtension()) && isInTestDirectory(file)) return true;
		return false;
	}

	public static boolean isTest(URI uri, String basePath) {
		if(Constants.EXTENSION.equals(uri.fileExtension()) && isInTestDirectory(uri, basePath)) return true;
		return false;
	}
}
