...
public class RTaskI extends GeneratorI implements RTaskListener {
    ...
    public RTaskI(EmbeddedTaskI parent, Task task) {
		...
		String name = fullPath.substring(0, fullPath.lastIndexOf(".")).substring(fullPath.lastIndexOf("/") + 1);
        ...
        // Parameters
        //
        for(Entry<String, String> p : ((org.ruminaq.tasks.rtask.model.rtask.RTask) task).getParameters().entrySet())
		...
		try {
			...
			this.fRservi.evalVoid("instance$rtListener <- RTListener$new('" + this.id + "')", null);

			this.fRservi.evalVoid("thriftc <- thriftr::load(t[0], module_name='runnersideserver_thrift', include_dirs=[t[1]])", null);
			this.fRservi.evalVoid("instance$rtListener$client <- thriftr::make_client(thriftc.RunnerSideServer, '127.0.0.1', " + CallbackServer.INSTANCE.getPort() + ")", null);
        ...
        //
		try {
			this.atomic         = this.fRservi.evalData("instance$rtaskinfo$atomic",          null).getData().getInt(0) == 1;
			this.generator      = this.fRservi.evalData("instance$rtaskinfo$generator",       null).getData().getInt(0) == 1;
			this.externalSource = this.fRservi.evalData("instance$rtaskinfo$external_source", null).getData().getInt(0) == 1;
			this.constant       = this.fRservi.evalData("instance$rtaskinfo$constant",        null).getData().getInt(0) == 1;
		} catch (CoreException e) {
			e.printStackTrace();
		}
    }
    ...
    @Override protected void executeConstant()                       {
    ...
    @Override protected void executeAsync(String portId, DataI data) {
    ...
    @Override protected void executeExternalSrc() {
    ...
    @Override public void sleep       (long l) { super.sleep(l); }
    @Override public void externalData(int i)  {
        addExternalSrcExecNb(i);
        setReadyWithParents(true);
    }
    @Override public void    generatorPause()    { super.pause(); }
    @Override public boolean generatorIsPaused() { return super.isPaused(); }
    @Override public void    generatorResume()   { super.resume(); }
    @Override public void    generatorEnd()      { super.end(); }
    @Override public void    exitRunner()        { super.breakRunner(); }

    @Override public String getParameter(String key)         { return parameters.get(key); }
    @Override public String runExpression(String expression) { return GroovyExpressionUtil.evaluate(expression).toString(); }
    ...
    @Override public DataI getData(String ipName, String datatype) {
        if(portIdData == null) return null;
    ...
    @Override public void putData(String opName, DataI dataI) {
