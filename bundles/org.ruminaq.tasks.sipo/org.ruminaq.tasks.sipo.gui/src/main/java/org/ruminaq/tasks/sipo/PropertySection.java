...
public class PropertySection implements IPropertySection {
    ...
    private void initLayout(Composite parent) {
        ...
        lytSize.minimumWidth = 75; lytSize.widthHint = 75;
                ...
                if(event.detail == SWT.TRAVERSE_RETURN) {
                    ModelUtil.runModelChange(new Runnable() {
                        public void run() {
                            ...
                            boolean parse = GroovyExpressionUtil.isOneDimIntegerAlsoGV(txtSize.getText());
                            if(parse) {
                                Object bo = Graphiti.getLinkService().getBusinessObjectForLinkedPictogramElement(pe);
                                InternalTransaction a = ((InternalTransactionalEditingDomain)ed).getActiveTransaction();
                                if(a == null || a.isReadOnly() == true) return;
                            ...
                            } else MessageDialog.openError(shell, "Can't edit value", "Don't understant numeric value");
                        }
                    }, ed, "Model Update");
                }
            }});
                ...
                ModelUtil.runModelChange(new Runnable() {
                    public void run() {
                        ...
                        if(bo instanceof Sipo) {
                            ...
                            if(btnIdx.getSelection()) { txtSize.setVisible(true);  spnSize.setVisible(false); txtSize.setText(Integer.toString(spnSize.getSelection())); }
                            else                      { txtSize.setVisible(false); spnSize.setVisible(true);
                                if(NumericUtil.isOneDimPositiveInteger(txtSize.getText())) spnSize.setSelection(Integer.parseInt(txtSize.getText()));
                                else                                                       element.setSize(Integer.toString(spnSize.getSelection()));
                            }
                            UpdateContext context = new UpdateContext(pe);
                            dtp.getFeatureProvider().updateIfPossible(context);
                            if(btnIdx.getSelection()) btnTrig.setEnabled(false);
                            else                      btnTrig.setEnabled(true);
                        }
                    }
                }, ed, "Model Update");
            }});
    ...
    public void refresh(PictogramElement pe, TransactionalEditingDomain ed) {
        if(pe != null) {
            ...
            if(sipo.isIndex()) { stkSize.topControl = txtSize; txtSize.setText(sipo.getSize()); }
            else               { stkSize.topControl = spnSize; spnSize.setSelection(Integer.parseInt(sipo.getSize())); }
            cmpSize.layout();
