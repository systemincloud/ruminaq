...
public class TasksDebugModelPresentation implements IDebugModelPresentation {
	...
	private static final Image TARGET_SUSPENDED        = getImage("target_suspended.png");
	...
	private static final Image TASK_PARTLY_SUSPENDED   = getImage("task_partly_suspended.png");
	...
	public Image getImage(Object element) {
		if(element instanceof TasksDebugTarget) {
			...
			if(target.isSuspended())  return TARGET_SUSPENDED;
			if(target.hasSuspended()) return TARGET_PARTLY_SUSPENDED;
			if(target.isStepping())   return TARGET_STEPPING;
			return TARGET_RUNNING;
		} else if(element instanceof Task) {
			Task task = (Task) element;
			if(task.isTerminated()) return TASK_TERMINATED;
			if(task.isSuspended())  return TASK_SUSPENDED;
			if(task.hasSuspended()) return TASK_PARTLY_SUSPENDED;
			if(task.isStepping())   return TASK_STEPPING;
			return TASK_RUNNING;
		} else if(element instanceof OutputPort) {
			OutputPort port = (OutputPort) element;
			if(port.isTerminated()) return OUT_TERMINATED;
			if(port.isSuspended())  return OUT_SUSPENDED;
			if(port.isStepping())   return OUT_STEPPING;
			return OUT_RUNNING;
		} else if(element instanceof InputPort) {
			InputPort port = (InputPort) element;
			if(port.isTerminated()) return IN_TERMINATED;
			if(port.isSuspended())  return IN_SUSPENDED;
			if(port.isStepping())   return IN_STEPPING;
			return IN_RUNNING;
