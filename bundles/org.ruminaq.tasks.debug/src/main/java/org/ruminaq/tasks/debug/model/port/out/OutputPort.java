...
public class OutputPort extends Port {
	...
	public OutputPort(IDebugTarget target, String id, Task task) {
		...
		waiting    = new StringVariable(getDebugTarget(), "waiting",     "false", false, this, MainState.SUSPENDED);
		waitingFor = new StringVariable(getDebugTarget(), "waiting for", "",      false, this, MainState.SUSPENDED);
	}
	...
	@Override public String  getName()       { return id + " [OUT]"; }
	...
	public void handleEvent(IDebugEvent event) {
		...
		if(event instanceof InternalOutputPortVariablesEvent && ((AbstractPortEvent) event).compare(this)) {
			setVariables(((InternalOutputPortVariablesEvent) event).getVariables());
		} else if(event instanceof DataEvent && ((AbstractPortEvent) event).compare(this)) {
			this.data = new Data(getDebugTarget(), ((DataEvent) event).getData(), this);
			fireChangeEvent(DebugEvent.CONTENT);
		} else if(event instanceof WaitingForEvent && ((AbstractPortEvent) event).compare(this)) {
	...
	@Override protected void suspended() { this.dataRetrived = false;
	                                       this.waitingForRetrived = false; }

	public synchronized void setVariables(Map<Variable, Object> map) {
		for(Variable name : map.keySet()) {
			if(name.equals(Variable.IS_SYNC)) config.add(Variable.IS_SYNC.getName(), (String) map.get(Variable.IS_SYNC), false, null);
		}
	}
